...
/**
 *@example ex_file.c
 *@brief Nilorea Library N_FILE api test
...
error:
    ...
    /* test n_scan_dir and n_free_file_info from n_files.h */
    LIST* file_list = new_generic_list(0);
    if (n_scan_dir(".", file_list, 0) == TRUE) {
        n_log(LOG_INFO, "n_scan_dir found %zu files in current directory", file_list->nb_items);
        list_foreach(fnode, file_list) {
            N_FILE_INFO* fi = (N_FILE_INFO*)fnode->ptr;
            if (fi && fi->name) {
                n_log(LOG_DEBUG, "  file: %s (time: %ld)", fi->name, (long)fi->filetime);
            }
        }
    } else {
        n_log(LOG_ERR, "n_scan_dir failed");
    }
