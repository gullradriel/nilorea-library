/**
 *@example ex_network.h
 *@brief Nilorea Library n_network api test
...
#ifdef __windows__
    if (gethostname(hostname->data, (int)hostname->length) != 0) {
#else
    if (gethostname(hostname->data, hostname->length) != 0) {
#endif
        n_log(LOG_ERR, "Couldn't resolve hostname. Error:%s", strerror(errno));
        free_nstr(&hostname);
        return FALSE;
    }
...
void* manage_client(void* ptr) {
    ...
    while (!DONE) {
        if ((netw_exchange = netw_get_msg(netw))) {
            ...
            if (type == -1) {
                n_log(LOG_ERR, "Error getting message type from network exchange");
