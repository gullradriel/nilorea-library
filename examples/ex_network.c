...
int main(int argc, char **argv)
{
    ...
    process_args( argc, argv, &addr, &srv, &port,&NB_ATTEMPTS, &ip_version, &ssl_key_file, &ssl_cert_file );
    ...
    if( mode == SERVER )
    {
        ...
        {
            netw_set_crypto( server , ssl_key_file , ssl_cert_file );
        }
#endif
    ...
    else if( mode == CLIENT )
    {
        for( int it = 1 ; it <= NB_ATTEMPTS ; it ++ )
        {
            ...
            {
#ifdef HAVE_OPENSSL
                if( netw_ssl_connect( &netw, srv, port, ip_version, ssl_key_file, ssl_cert_file ) != TRUE )
                {
