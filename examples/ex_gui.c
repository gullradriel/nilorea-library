 ...
 *
 * The main loop also demonstrates the opt-in redraw signal: it draws only when
 * n_gui_needs_redraw() reports a change or animation (or when this example's own
 * mouse crosshair overlay moved), and pairs its timer-driven label updates with
 * n_gui_mark_dirty(), so an untouched, still GUI is not redrawn every frame.
 *
...
void on_save_layout_click(int widget_id, void* user_data) {
    ...
    if (n_gui_save_layout_json(gui, EX_GUI_LAYOUT_FILE) == 0) {
...
void on_load_layout_click(int widget_id, void* user_data) {
    (void)widget_id;
    N_GUI_CTX* gui = (N_GUI_CTX*)user_data;
    if (n_gui_load_layout_json(gui, EX_GUI_LAYOUT_FILE) == 0) {
...
int main(int argc, char* argv[]) {
    ...
    n_gui_window_set_flags(gui, win_scroll, N_GUI_WIN_AUTO_SCROLLBAR | N_GUI_WIN_NO_HSCROLL | N_GUI_WIN_RESIZABLE);
    win_idx++;
     ...
     * titlebar Close button. */
    {
        ...
        n_gui_window_set_tb_button_bitmaps(gui, win_skin2, N_GUI_TB_BTN_CLOSE,
    ...
    n_gui_window_set_resize_policy(gui, win1, N_GUI_WIN_RESIZE_MOVE);
    n_gui_window_set_resize_policy(gui, win2, N_GUI_WIN_RESIZE_SCALE);
    n_gui_window_set_resize_policy(gui, win3, N_GUI_WIN_RESIZE_NONE);
    n_gui_window_set_resize_policy(gui, win4, N_GUI_WIN_RESIZE_MOVE);
    n_gui_window_set_resize_policy(gui, win5, N_GUI_WIN_RESIZE_SCALE);
    n_gui_window_set_resize_policy(gui, win6, N_GUI_WIN_RESIZE_SCALE);
    n_gui_window_set_resize_policy(gui, win7, N_GUI_WIN_RESIZE_SCALE);
    n_gui_window_set_resize_policy(gui, win8, N_GUI_WIN_RESIZE_SCALE);
    n_gui_window_set_resize_policy(gui, win9, N_GUI_WIN_RESIZE_SCALE);
    n_gui_window_set_resize_policy(gui, win10, N_GUI_WIN_RESIZE_MOVE);
    n_gui_window_set_resize_policy(gui, win11, N_GUI_WIN_RESIZE_MOVE);
    n_gui_window_set_resize_policy(gui, win12, N_GUI_WIN_RESIZE_SCALE);
    n_gui_window_set_resize_policy(gui, win_scroll, N_GUI_WIN_RESIZE_SCALE);
    ...
    do {
        ...
        if (ev.type == ALLEGRO_EVENT_TIMER) {
            ...
            if (strcmp(last_coord, coord_buf) != 0) {
                n_gui_label_set_text(gui, lbl_coords, coord_buf);
