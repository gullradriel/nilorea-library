...
if [ -f ./ex_network_sni ]; then
    asan_test "ex_network_sni"
fi
...
# n_gui event re-entrancy regression (headless). A button on_click that raises
# another window reorders ctx->windows mid mouse-release iteration; asserts the
# deferred-callback dispatch survives it (use-after-free under ASan otherwise).
if [ -f ./ex_gui_reentrant ]; then
    asan_test "ex_gui_reentrant"
fi

# n_gui new-widget headless regressions (split pane, hex view, syntax view,
...
if [ -f ./ex_gui_syntaxview ]; then
    asan_test "ex_gui_syntaxview"
fi
if [ -f ./ex_gui_dropmenu ]; then
    asan_test "ex_gui_dropmenu"
fi
# n_gui layout widget-state persistence regression (headless): a keyed splitpane
# ratio + datagrid column layout survive save/load into a fresh context, incl. the
# lazily-created-widget (pending) path.
if [ -f ./ex_gui_layout_persist ]; then
    asan_test "ex_gui_layout_persist"
fi
if [ -f ./ex_gui_datagrid ]; then
    asan_test "ex_gui_datagrid"
fi
if [ -f ./ex_gui_progressbar ]; then
    asan_test "ex_gui_progressbar"
fi
if [ -f ./ex_gui_placeholder ]; then
    asan_test "ex_gui_placeholder"
fi
if [ -f ./ex_gui_zorder ]; then
    asan_test "ex_gui_zorder"
fi
# n_gui native (detached) window regression. Needs a real display to create the
# second window, so it skips itself with a success exit code when there is no
# X server (headless CI). Run it under xvfb-run to actually exercise it.
if [ -f ./ex_gui_multiwin ]; then
    asan_test "ex_gui_multiwin"
fi
...
# URL canonicalization test (no network needed)
asan_test "ex_url"

# HTML/sitemap extraction test (no network needed)
asan_test "ex_html"

# HTTP/2 frame codec + HPACK regression (RFC 7541 vectors, no network needed)
asan_test "ex_http2"

# Mock HTTP server test (plain HTTP, no SSL needed)
asan_test "ex_network_mock"

# Interruptible connect: an abort callback cuts short a connect to an
# unresponsive (RFC 5737 blackhole) host instead of waiting the timeout.
asan_test "ex_network_connect_abort"
