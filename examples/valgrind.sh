...
# NO GFX list
test_list="ex_log ex_configfile ex_exceptions ex_hash ex_list ex_monolith ex_network ex_nstr ex_pcre ex_threads"

for example in `echo $test_list`
do
	echo "##########"
	echo "Testing $example"
	EXEMPLE_OUTPUT=`valgrind --leak-check=full --show-leak-kinds=all --track-origins=yes --verbose --log-file=${example}_debugmem.txt ./${example} -V LOG_DEBUG 2>&1`
	EXEMPLE_RET=$?
