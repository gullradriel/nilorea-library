...
/*! @brief evaluate the peer certificate (chain trust + host match) against the system trust store, independently of the connection verify mode */
...
/*! @brief SNI cert-pick callback for netw_accept_ssl_with_sni_cb: given the
 *  client SNI host (may be NULL), fill *cert_pem and *key_pem with a server
 *  certificate and key (newly allocated; n_network frees them). Return 0 to
 *  proceed with the handshake, non-zero to fail it. */
typedef int (*netw_sni_pick_cb)(const char* sni, N_STR** cert_pem, N_STR** key_pem, void* user_data);
/*! @brief get the SNI server_name the peer requested in its ClientHello, or NULL */
const char* netw_ssl_get_sni(NETWORK* netw);
/*! @brief install an in-memory cert/key on THIS connection's SSL only (per-connection, unlike the context-wide netw_set_crypto_pem) */
int netw_set_crypto_pem_ctx(NETWORK* netw, const N_STR* key_pem, const N_STR* cert_pem);
/*! @brief accept a TLS connection, selecting the server certificate per client SNI via pick() during the handshake */
NETWORK* netw_accept_ssl_with_sni_cb(NETWORK* listen, netw_sni_pick_cb pick, void* user_data);
/*! @brief upgrade an already-accepted plaintext connection to a TLS server, selecting the cert per client SNI via pick() during the handshake (for proxy CONNECT) */
...
/*! @brief set up a client SSL_CTX on an already-connected NETWORK (no handshake yet) */
...
/*! @brief register a process-wide callback polled during connect; non-zero aborts the in-progress connect (NULL removes) */
void netw_set_connect_abort_cb(int (*cb)(void* ctx), void* ctx);
/*! @brief adopt an already-connected socket fd into a fresh client NETWORK (plain send/recv) */
...
/*! @brief leading digit of an HTTP status code (1..5), or 0 when outside 100..599 */
int n_http_status_class(int status_code);
/*! @brief 1 when status_code is informational (1xx), else 0 */
int n_http_status_is_informational(int status_code);
/*! @brief 1 when status_code is success (2xx), else 0 */
int n_http_status_is_success(int status_code);
/*! @brief 1 when status_code is a redirect (3xx), else 0 */
int n_http_status_is_redirect(int status_code);
/*! @brief 1 when status_code is a client error (4xx), else 0 */
int n_http_status_is_client_error(int status_code);
/*! @brief 1 when status_code is a server error (5xx), else 0 */
int n_http_status_is_server_error(int status_code);
/*! @brief decompress an HTTP body per its Content-Encoding ("gzip"/"deflate"; identity/NULL/unknown copies through). Sets *out to a new N_STR (free with free_nstr). Returns 0 on success, -1 on error. */
...
typedef struct N_HTTP_MULTIPART_PART {
...
/*! @brief copy the boundary token from a Content-Type value into out (without the leading "--"). Returns 0 on success, -1 when absent. */
int n_http_multipart_boundary(const char* content_type, char* out, size_t outsz);
/*! @brief parse a multipart body (boundary given without "--") into a LIST of N_HTTP_MULTIPART_PART*; free with n_http_multipart_free. Returns NULL on a bad argument. */
LIST* n_http_parse_multipart(const N_STR* body, const char* boundary);
/*! @brief build a multipart/form-data body from a LIST of N_HTTP_MULTIPART_PART* using boundary (without "--"); returns a new N_STR (free with free_nstr) or NULL. */
N_STR* n_http_build_multipart(LIST* parts, const char* boundary);
/*! @brief free a LIST returned by n_http_parse_multipart and NULL the pointer */
...
/*! @brief produce a canonical URL string for dedup (lowercase scheme/host, drop default port, remove ./ and ../ segments, keep query) */
...
/*! @brief resolve a (possibly relative) reference against an absolute base URL (RFC 3986 section 5) */
...
/*! A single parsed WebSocket frame (RFC 6455). payload points into the parsed
 *  buffer and is still masked when masked is set; use n_ws_unmask to decode it. */
typedef struct N_WS_FRAME {
...
/*! @brief parse one WebSocket frame from a byte buffer (stateless). Returns 1 and
 *  fills frame + *consumed (total frame bytes) on a complete frame, 0 when more
 *  bytes are needed, -1 on a malformed or over-large frame. payload aliases buf. */
int n_ws_frame_parse(const unsigned char* buf, size_t len, N_WS_FRAME* frame, size_t* consumed);

/*! @brief XOR-unmask len bytes of src into dst using a 4-byte WebSocket mask key */
void n_ws_unmask(unsigned char* dst, const unsigned char* src, size_t len, const unsigned char mask[4]);

/*! @brief build a WebSocket frame into out (optionally masking the payload).
