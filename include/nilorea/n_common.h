...
#define Malloc(__ptr, __struct, __size)                                                                                                                                                        \
    {                                                                                                                                                                                          \
        ...
        __n_errno = errno;                                                                                                                                                                     \
        if (!__ptr) {                                                                                                                                                                          \
            n_log(LOG_ERR, "( %s *)calloc( %zu , sizeof( %s ) ) %s at line %d of %s", #__ptr, __size, #__struct, (__n_errno == 0) ? "malloc error" : strerror(__n_errno), __LINE__, __FILE__); \
        }                                                                                                                                                                                      \
    }
...
#define Alloca(__ptr, __size)                                                                                                                                    \
    {                                                                                                                                                            \
        ...
        __n_errno = errno;                                                                                                                                       \
        if (!__ptr) {                                                                                                                                            \
            n_log(LOG_ERR, "%s=alloca( %zu ) %s at line %d of %s", #__ptr, __size, (__n_errno == 0) ? "alloca error" : strerror(__n_errno), __LINE__, __FILE__); \
        } else {                                                                                                                                                 \
            memset(__ptr, 0, __size);                                                                                                                            \
        }                                                                                                                                                        \
    }

/*! Realloc Handler to get errors. Returns TRUE on success, FALSE on failure.
    On failure the original pointer is preserved (not set to NULL). */
#define Realloc(__ptr, __struct, __size)                                                                                                                                                         \
    ({                                                                                                                                                                                           \
        int __n_errno = 0;                                                                                                                                                                       \
        errno = 0;                                                                                                                                                                               \
        void* __new_ptr = ((__size) && (sizeof(__struct)) && ((__size) > SIZE_MAX / sizeof(__struct))) ? NULL : (__struct*)realloc(__ptr, (__size) * sizeof(__struct));                          \
        __n_errno = errno;                                                                                                                                                                       \
        if (!__new_ptr) {                                                                                                                                                                        \
            n_log(LOG_ERR, "( %s *)realloc( %s * sizeof( %zu ) ) %s at line %d of %s", #__ptr, #__struct, __size, (__n_errno == 0) ? "realloc error" : strerror(__n_errno), __LINE__, __FILE__); \
        } else {                                                                                                                                                                                 \
            __ptr = __new_ptr;                                                                                                                                                                   \
        }                                                                                                                                                                                        \
        (__new_ptr != NULL) ? TRUE : FALSE;                                                                                                                                                      \
    })

/*! Realloc + zero new memory zone Handler to get errors. Returns TRUE on success, FALSE on failure.
    On failure the original pointer is preserved (not set to NULL). */
#define Reallocz(__ptr, __struct, __old_size, __size)                                                                                                                                            \
    ({                                                                                                                                                                                           \
        int __n_errno = 0;                                                                                                                                                                       \
        errno = 0;                                                                                                                                                                               \
        void* __new_ptr = ((__size) && (sizeof(__struct)) && ((__size) > SIZE_MAX / sizeof(__struct))) ? NULL : (__struct*)realloc(__ptr, __size);                                               \
        __n_errno = errno;                                                                                                                                                                       \
        if (!__new_ptr) {                                                                                                                                                                        \
            n_log(LOG_ERR, "( %s *)realloc( %s * sizeof( %zu ) ) %s at line %d of %s", #__ptr, #__struct, __size, (__n_errno == 0) ? "realloc error" : strerror(__n_errno), __LINE__, __FILE__); \
        } else {                                                                                                                                                                                 \
            __ptr = __new_ptr;                                                                                                                                                                   \
            if (__size > __old_size) memset(((char*)__ptr + __old_size), 0, __size - __old_size);                                                                                                \
        }                                                                                                                                                                                        \
        (__new_ptr != NULL) ? TRUE : FALSE;                                                                                                                                                      \
    })
...
/* On Windows, Winsock (winsock2.h) already declares htonll/ntohll as functions.
   Defining them as macros here makes those later function declarations fail to parse
   (the macro expands inside the prototype), so the 64-bit byte-order fallbacks are only
   defined on non-Windows platforms; Windows code uses Winsock's htonll/ntohll. */
#ifndef __windows__
...
/*! @brief exit and print log to stderr */
void n_abort(char const* format, ...);

/*! @brief get the computer name into the given buffer */
int get_computer_name(char* computer_name, size_t len);

/*! @brief get running program name */
char* get_prog_name(void);

/*! @brief get running program directory */
char* get_prog_dir(void);

/*! @brief test file presence */
int file_exist(const char* filename);

/*! @brief shortcut for popen usage */
int n_popen(char* cmd, size_t read_buf_size, void** nstr_output, int* ret);

/*! @brief log environment variables at the given log level */
void log_environment(int loglevel);

#ifndef __windows__
/*! @brief reap zombie processes */
void sigchld_handler(int sig);
/*! @brief install signal SIGCHLD handler */
...
/*! @brief daemonize the current process with the given mode flags */
int n_daemonize_ex(int mode);
/*! @brief non-blocking system call */
pid_t system_nb(const char* command, int* infp, int* outfp);
#endif

/*! @brief reconstruct hidden string from variadic char arguments */
