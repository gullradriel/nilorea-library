...
/*! @brief thread-safe regex substitute: replace every match in subject (binary-safe,
 *  subject_len bytes) using a replacement template with $N / ${N} capture-group
 *  references (pcre2_substitute, global). Returns a new N_STR (free with free_nstr),
 *  or NULL on error. */
