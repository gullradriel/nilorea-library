...
/*! @brief create a new empty n-ary TREE */
TREE* new_tree();
/*! @brief create a TREE_NODE with the given value and optional destructor */
TREE_NODE* tree_create_node(NODE_DATA value, void (*destroy_func)(void* ptr));
/*! @brief insert a child node under the given parent node */
int tree_insert_child(TREE_NODE* parent, TREE_NODE* child);
/*! @brief delete a TREE_NODE and all its children from the tree */
int tree_delete_node(TREE* tree, TREE_NODE* node);
/*! @brief destroy a TREE and all its nodes, set pointer to NULL */
...
/*! @brief create a new quadtree for the given coordinate type */
QUADTREE* create_quadtree(int coord_type);
/*! @brief create a new quadtree node with position and data pointer */
QUADTREE_NODE* create_node(COORD_VALUE x, COORD_VALUE y, void* data_ptr);
/*! @brief search for a point in the quadtree, return matching node or NULL */
QUADTREE_NODE* search(QUADTREE* qt, QUADTREE_NODE* root, COORD_VALUE x, COORD_VALUE y);
/*! @brief insert a point with data into the quadtree */
void insert(QUADTREE* qt, QUADTREE_NODE** root, COORD_VALUE x, COORD_VALUE y, void* data_ptr);
/*! @brief recursively free all nodes of a quadtree */
...
/*! @brief create a new octree for the given coordinate type */
OCTREE* create_octree(int type);
/*! @brief create a new octree node with position and data pointer */
OCTREE_NODE* create_octree_node(POINT3D point, void* data_ptr);
/*! @brief insert a point with data into the octree */
void insert_octree(OCTREE* OCTREE, POINT3D point, void* data_ptr);
/*! @brief recursively free an octree node and its children */
void free_octree_node(OCTREE_NODE* node);
/*! @brief free the entire octree and its root */
