...
/*! @brief Read lifecycle counters.  Both pointers optional.  Monotonic
 *  since process start; advisory (not thread-atomic). */
void netw_msg_stats_get(long long* created, long long* deleted);

/*! @brief Read process-wide network byte counters, sum of
 *  `N_STR.written` across every netw_add_msg / netw_get_msg call.
 *  Includes wire payload only; no framing overhead.  Either pointer
 *  may be NULL.  Advisory (not thread-atomic). */
...
/*! @brief add a binary blob to the message, clean API for raw payloads
 * (e.g. packed entity wires, batched snapshots) that should not be
 * confused with character strings. Internally wraps add_nstrptr_to_msg
 * so the wire format is identical to a regular string today; the
 * distinction exists for caller clarity and for a future framing
 * compaction (the redundant N_STR `length` field could be removed
 ...
 * add_bytes_to_msg). Allocates `*out_bytes` (caller frees with
 * `free`) and writes its size into `*out_len`. Returns TRUE on
 * success, FALSE on missing entry or empty payload. */
