...
/*! Search-window margin (in cells) added around the bounding box of the
 *  start and goal. n_astar_find_path() allocates its per-call working set
 *  for this window only, not the whole grid, so a short path on a huge map
 *  costs a few KB instead of width*height*depth cells. A route that must
 *  detour more than this many cells outside the start-goal box will not be
 *  found (the call returns NULL). Raise it if you need wider detours at the
 *  cost of a larger per-call allocation. */
#ifndef ASTAR_SEARCH_MARGIN
#define ASTAR_SEARCH_MARGIN 128
#endif
