 ...
 * per-cell height values, configurable projection, and terrain
 * transition support. */
typedef struct ISO_MAP {
    ...
    /* neighbor edge terrains for chunked transition masks
     * When a tile sits at a chunk edge, iso_map_calc_transitions_full
     * needs to know the terrain type on the far side to decide whether
     * to blit a bleed mask. Without these, the out-of-bounds terrain
     * lookup returns 0 (water), so every cross-chunk transition is
     * silently dropped and a hard diagonal seam appears along the
     * chunk boundary. NULL means the chunk is on the world boundary
     * or the neighbor is not resident; callers fall back to the
     * previous "treat as base" behaviour. */
    int* neighbor_terrains_west;  /*!< [height], terrain of west neighbor's east edge */
    int* neighbor_terrains_north; /*!< [width],  terrain of north neighbor's south edge */
    int* neighbor_terrains_east;  /*!< [height], terrain of east neighbor's west edge */
    int* neighbor_terrains_south; /*!< [width],  terrain of south neighbor's north edge */
    ...
    /*! Cached draw_order. The qsort+build in
     *  `iso_map_build_draw_order` is expensive (1024+ entries x 25
     *  chunks per frame = ~3 ms in profiled bench). Cache the result
     *  on the map and only rebuild when segments or heights change.
     *  Setters that mutate either field (`iso_map_set_segments` is the
     *  only public one) set `draw_order_dirty = 1`. `iso_map_draw`
     *  rebuilds into `cached_draw_order` when dirty, then reuses across
     *  frames. Lifetime tied to the map; freed in `iso_map_free`. */
...
typedef struct N_ISO_OBJECT {
    ...
    float sprite_h;            /*!< entity height in world units for occlusion (derived from sprite hot_y) */
    float sprite_half_w;       /*!< entity half-width in screen pixels for horizontal occlusion test */
    int is_occluded;           /*!< OUTPUT: set to 1 if behind tiles during last iso_map_draw() call */
    float occlude_clip_y;      /*!< OUTPUT: screen Y of top occluder north vertex (for cross-chunk clip fallback) */
...
/**
 * @brief Per-pass timing accumulators populated by iso_map_draw.
 *
 * Each iso_map_draw() call adds its per-pass elapsed time (microseconds)
 * to these globals. The host application is responsible for resetting
 * them to 0.0 at the start of every measurement window (typically frame
 * start), then reading them after the world render block to see how the
 * cost split across the three internal passes:
 *
 *   - n_iso_pass1_us: Pass 1: ground tiles (object-free loop)
 *   - n_iso_pass2_us: Pass 2: elevated segments + interleaved object
 *                             callbacks (includes time spent inside
 *                             N_ISO_OBJECT.draw callbacks; subtract any
 *                             host-side callback timing to isolate the
 *                             library's segment work)
 *   - n_iso_pass3_us: Pass 3: occlusion ghost overlay
 *
 * Always populated. The overhead is six al_get_time() calls per
 * iso_map_draw invocation: empirically < 1 us total, negligible vs the
 * milliseconds-scale work the function does. No runtime flag needed.
 */
extern double n_iso_pass1_us;
extern double n_iso_pass2_us;
extern double n_iso_pass3_us;
/* Pre-pass setup cost inside iso_map_draw: draw_order alloc +
 * iso_map_build_draw_order + object screen-pos compute + insertion
 * sort. Surfaced separately from Pass 1/2/3 because in profiled runs
 * this section can be > 50 % of the total iso_map_draw cost when
 * objects are dense. */
extern double n_iso_setup_us;

/*! Optional design-RGB to display-RGB filter applied to every
 * primitive color (filled polygons, lines) emitted by `iso_map_draw`.
 * Used by the client to opt into sRGB-correct rendering: when an
 * sRGB FBO is bound and GL_FRAMEBUFFER_SRGB is enabled, RGB literals
 * authored in sRGB space must be hand-converted to linear before
 * being written, otherwise the GPU's linear-to-sRGB encode at FBO
 * store time would double-encode them. Default NULL = identity
 * passthrough; non-sRGB consumers (editor, headless tests) pay
 * nothing. Set via `iso_set_primitive_color_filter`. */
typedef ALLEGRO_COLOR (*IsoPrimitiveColorFilter)(ALLEGRO_COLOR c);
...
/* Per-frame callback that returns the screen-space (dx, dy) offset
 * which the consumer has added to its world camera AFTER the iso
 * engine's `cam_px / cam_py` were sampled (e.g. a temporal-AA
 * sub-pixel jitter applied on top of the camera snap). The engine
 * subtracts this offset from the endpoints of every decorative line
 * draw (cliff-base edge lines in `_draw_segment_sides`; top-face
 * edges, vertical corner lines, and underside borders in
 * `_draw_height_borders`) so those lines stay pixel-stable across
 * frames while filled polygon faces and bitmap sprites continue to
 * inherit the jitter (which is what drives the temporal-AA averaging
 * on bitmap content). Without the compensation, thin anti-aliased
 * lines hit a different sub-pixel position each frame; the
 * rasterizer's AA coverage flickers; any temporal blend downstream
 * (TAA's history-weight composite) then averages two slightly
 * offset semi-transparent lines into visible grey/white pixels along
 * the line.
 *
 * Default NULL = no compensation. Consumers without a jitter source
 * pay nothing. The callback is invoked per-tile during
 * `iso_map_draw` (potentially hundreds of times per frame), so it
 * must be O(1): typically returns the cached jitter from a
 * `_begin_frame` call elsewhere. Returning (0, 0) is the "no
 * compensation this frame" signal and is the expected steady state
 * when the jitter source is off (e.g. camera stationary, TAA
 * disabled). */
typedef void (*IsoLineJitterCompensationFn)(float* out_dx, float* out_dy);
