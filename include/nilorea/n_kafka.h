...
/**
 *@file n_kafka.h
 *@brief Kafka generic produce and consume event header
...
typedef struct N_KAFKA_EVENT {
    ...
    /*! wall-clock time at which status flipped to N_KAFKA_EVENT_ERROR. 0 when the event has never errored. Used by the retry logic gated by N_KAFKA::error_timeout. */
    time_t error_time;
} N_KAFKA_EVENT;
...
typedef struct N_KAFKA {
    ...
    /*! polling thread status, 0 => off , 1 => on , 2 => wants to stop, will be turned out to 0 by exiting polling thread */
    ...
    /*! retry interval in seconds for events stuck in N_KAFKA_EVENT_ERROR. 0 (default) disables retries, errored events stay errored until the caller dumps and reloads them. */
    int32_t error_timeout;
    /*! TRUE when a 'transactional.id' is set in the producer config: produces are then wrapped in kafka transactions. FALSE keeps the legacy fire-and-forget produce path. */
    int is_transactional;
    /*! base directory the produced event files are read from ('to-send'). Used together with sended_dir to compute the destination subpath when moving a file on ack. NULL keeps the legacy behaviour. */
    char* tosend_dir;
    /*! base directory produced event files are moved to once acknowledged ('sended'). When NULL, acknowledged files are deleted (legacy behaviour) instead of being moved. */
    char* sended_dir;
} N_KAFKA;
...
/*! set the retry interval (in seconds) for errored events. 0 disables retries. */
int n_kafka_set_error_timeout(N_KAFKA* kafka, int32_t error_timeout);
/*! set the 'to-send' and 'sended' base directories used to move acknowledged event files instead of deleting them */
