...
/* forward declaration for n_str_template_expand (avoids pulling in n_hash.h) */
struct HASH_TABLE;
...
typedef struct N_STR {
    ...
    size_t length;
    /*! number of meaningful bytes in data, excluding the null terminator;
     *  the size including the null terminator is `written + 1`. */
    size_t written;
} N_STR;
...
/*! @brief copy from string to dest until from[iterator] == delim */
...
/*! @brief Expand double-brace tokens in a template using a hash table.
 *  Tokens not found in vars are left unchanged.
 *  @param tmpl Template string with {{key}} tokens. Must not be NULL.
 *  @param vars Hash table mapping key to value. May be NULL.
 *  @return Newly allocated N_STR with expanded result, or NULL on failure. */
N_STR* n_str_template_expand(const char* tmpl, struct HASH_TABLE* vars);

/*! @brief Percent-encode a string per RFC 3986 (unreserved chars kept).
 *
 *  Letters, digits and -_.~ are written verbatim; all other bytes become %HH.
 *  Uses explicit range checks (locale-independent). n_url_encode() in n_network.h
 *  is a convenience alias that delegates to this function. See the full
 *  parameter contract on the implementation in `src/n_str.c`. */
