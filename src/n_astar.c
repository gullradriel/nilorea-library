...
/*! Search window: the bounding box of (start, goal) expanded by
 *  ASTAR_SEARCH_MARGIN and clamped to the grid. The per-call ASTAR_CELL
 *  working set is allocated for this window only, NOT the whole grid, so a
 *  short path on a huge map costs a few KB instead of width*height*depth
 *  cells. The grid's own walkable[] / cost[] planes stay full-size and are
 *  still indexed by the absolute grid_index(). */
typedef struct ASTAR_BOX {
    int minx, miny, minz; /*!< window origin (absolute grid coords) */
    int bw, bh, bd;       /*!< window dimensions in cells */
} ASTAR_BOX;

/*! Flat index into the window-local ASTAR_CELL array */
static inline int box_index(const ASTAR_BOX* b, int x, int y, int z) {
    return ((z - b->minz) * b->bh + (y - b->miny)) * b->bw + (x - b->minx);
}

/*! Check if coordinates fall inside the search window */
static inline int in_box(const ASTAR_BOX* b, int x, int y, int z) {
    return x >= b->minx && x < b->minx + b->bw &&
           y >= b->miny && y < b->miny + b->bh &&
           z >= b->minz && z < b->minz + b->bd;
}
...
static ASTAR_PATH* reconstruct_path(ASTAR_CELL* cells, const ASTAR_BOX* box, int sx, int sy, int sz, int gx, int gy, int gz) {
    ...
    while (cx != sx || cy != sy || cz != sz) {
        length++;
        int idx = box_index(box, cx, cy, cz);
        ...
        cy = py;
        cz = pz;
        if (length > box->bw * box->bh * box->bd) {
            return NULL;
        }
    }
    length++;
    ...
    path->length = length;
    path->cost = cells[box_index(box, gx, gy, gz)].g;
...
ASTAR_PATH* n_astar_find_path(const ASTAR_GRID* grid,
    ...
    /* Window the per-call working set to the bounding box of (start, goal)
     * expanded by ASTAR_SEARCH_MARGIN and clamped to the grid, instead of
     * allocating one ASTAR_CELL per map tile. A 2400x2400 grid is 5.76M
     * cells (~161 MB) per calloc/free — fired on every chase repath, this
     * was the cause of multi-hundred-ms tick spikes. The grid's persistent
     * walkable[] / cost[] planes stay full-size and keep using the absolute
     * grid_index(); only the transient `cells` scratch is windowed, indexed
     * by box_index(). So per-call cost scales with the path length, not the
     * map size: a few KB for a short path. Trade-off: an optimal route that
     * detours more than ASTAR_SEARCH_MARGIN tiles outside the start-goal box
     * is not found through this window (returns NULL). For bounded game
     * paths that never happens; raise ASTAR_SEARCH_MARGIN if a caller needs
     * wider detours.
     *
     * calloc keeps zero-initialised cells (ASTAR_NODE_NONE == 0 == the
     * "unvisited" state); the search only touches the handful of cells it
     * explores, and reconstruct_path walks parents it has already written. */
    ASTAR_BOX box;
    {
        int lo = (sx < gx) ? sx : gx, hi = (sx > gx) ? sx : gx;
        box.minx = lo - ASTAR_SEARCH_MARGIN; if (box.minx < 0) box.minx = 0;
        hi += ASTAR_SEARCH_MARGIN; if (hi > grid->width - 1) hi = grid->width - 1;
        box.bw = hi - box.minx + 1;

        lo = (sy < gy) ? sy : gy; hi = (sy > gy) ? sy : gy;
        box.miny = lo - ASTAR_SEARCH_MARGIN; if (box.miny < 0) box.miny = 0;
        hi += ASTAR_SEARCH_MARGIN; if (hi > grid->height - 1) hi = grid->height - 1;
        box.bh = hi - box.miny + 1;

        if (is_3d) {
            lo = (sz < gz) ? sz : gz; hi = (sz > gz) ? sz : gz;
            box.minz = lo - ASTAR_SEARCH_MARGIN; if (box.minz < 0) box.minz = 0;
            hi += ASTAR_SEARCH_MARGIN; if (hi > grid->depth - 1) hi = grid->depth - 1;
            box.bd = hi - box.minz + 1;
        } else {
            box.minz = 0;
            box.bd = 1;
        }
    ...
    size_t btotal = (size_t)box.bw * (size_t)box.bh * (size_t)box.bd;
    ASTAR_CELL* cells = (ASTAR_CELL*)calloc(btotal, sizeof(ASTAR_CELL));
    ...
    while (open->size > 0) {
        ...
        int ci = grid_index(grid, cx, cy, cz);   /* absolute: cost[] */
        int lci = box_index(&box, cx, cy, cz);   /* window-local: cells[] */

        if (cells[lci].status == ASTAR_NODE_CLOSED)
            continue;

        if (cx == gx && cy == gy && cz == gz) {
            result = reconstruct_path(cells, &box, sx, sy, sz, gx, gy, gz);
            break;
        }

        cells[lci].status = ASTAR_NODE_CLOSED;
        ...
        if (is_3d) {
            ...
            for (int d = 0; d < 6; d++) {
                ...
                if (!in_box(&box, nx, ny, nz)) continue;

                int ni = grid_index(grid, nx, ny, nz);
                int lni = box_index(&box, nx, ny, nz);
                if (!grid->walkable[ni] || cells[lni].status == ASTAR_NODE_CLOSED)
                    continue;

                int move_cost = (grid->cost[ci] + grid->cost[ni]) / 2;
                int tentative_g = cells[lci].g + move_cost;

                if (cells[lni].status == ASTAR_NODE_NONE || tentative_g < cells[lni].g) {
                    cells[lni].g = tentative_g;
                    cells[lni].h = n_astar_heuristic(nx, ny, nz, gx, gy, gz, heuristic);
                    cells[lni].f = tentative_g + cells[lni].h;
                    cells[lni].parent_x = cx;
                    cells[lni].parent_y = cy;
                    cells[lni].parent_z = cz;
                    cells[lni].status = ASTAR_NODE_OPEN;
                    heap_push(open, nx, ny, nz, cells[lni].f);
                }
            }
            ...
            for (int d = 0; d < 4; d++) {
                ...
                if (!in_box(&box, nx, ny, 0)) continue;

                int ni = grid_index(grid, nx, ny, 0);
                int lni = box_index(&box, nx, ny, 0);
                if (!grid->walkable[ni] || cells[lni].status == ASTAR_NODE_CLOSED)
                    continue;
                ...
                if (cells[lni].status == ASTAR_NODE_NONE || tentative_g < cells[lni].g) {
                    cells[lni].g = tentative_g;
                    cells[lni].h = n_astar_heuristic(nx, ny, 0, gx, gy, 0, heuristic);
                    cells[lni].f = tentative_g + cells[lni].h;
                    cells[lni].parent_x = cx;
                    cells[lni].parent_y = cy;
                    cells[lni].parent_z = 0;
                    cells[lni].status = ASTAR_NODE_OPEN;
                    heap_push(open, nx, ny, 0, cells[lni].f);
                }
            }
