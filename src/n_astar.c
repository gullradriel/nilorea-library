...
ASTAR_PATH* n_astar_find_path(const ASTAR_GRID* grid,
    ...
    ASTAR_BOX box;
    {
        ...
        box.minx = lo - ASTAR_SEARCH_MARGIN;
        if (box.minx < 0) box.minx = 0;
        hi += ASTAR_SEARCH_MARGIN;
        if (hi > grid->width - 1) hi = grid->width - 1;
        box.bw = hi - box.minx + 1;

        lo = (sy < gy) ? sy : gy;
        hi = (sy > gy) ? sy : gy;
        box.miny = lo - ASTAR_SEARCH_MARGIN;
        if (box.miny < 0) box.miny = 0;
        hi += ASTAR_SEARCH_MARGIN;
        if (hi > grid->height - 1) hi = grid->height - 1;
        box.bh = hi - box.miny + 1;

        if (is_3d) {
            lo = (sz < gz) ? sz : gz;
            hi = (sz > gz) ? sz : gz;
            box.minz = lo - ASTAR_SEARCH_MARGIN;
            if (box.minz < 0) box.minz = 0;
            hi += ASTAR_SEARCH_MARGIN;
            if (hi > grid->depth - 1) hi = grid->depth - 1;
