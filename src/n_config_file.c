...
CONFIG_FILE* load_config_file(char* filename, int* errors) {
    ...
    while (fgets(buffer, MAX_CONFIG_LINE_LEN, in)) {
        ...
        char* bufptr = trim_nocopy(buffer);
        if (strlen(bufptr) == 0) {
            continue;
        }
        ...
        switch (bufptr[0]) {
            ...
            default:
                ...
                if (npcre_match_capture(bufptr, npcre) == TRUE) {
                    Malloc(split_result, char*, 3);
                    if (!split_result) {
                        n_log(LOG_ERR, "allocation failed at line %zu of %s", line_number, filename);
                        (*errors)++;
                        continue;
                    }
                    split_result[2] = NULL;
                    split_result[0] = str_replace((char*)npcre->match_list[1], "=", " ");
                    split_result[1] = strdup((char*)npcre->match_list[2]);
                } else {
                    split_result = NULL;
                }
                ...
                if (strlen(trim_nocopy(split_result[0])) == 0) {
                    free_split_result(&split_result);
                    n_log(LOG_ERR, "couldn't find entry name at line %zu of %s (string:%s)", line_number, filename, buffer);
                    (*errors)++;
                    continue;
                }
                ...
                ht_put_ptr(section->entries, _nstr(entry_key), strptr, &free_no_null, NULL);
                free_nstr(&entry_key);
                free_split_result(&split_result);
                continue;
        }
...
int write_config_file(CONFIG_FILE* cfg_file, char* filename) {
    ...
    const char *section = NULL, *key = NULL, *val = NULL;
 ...
 *@return The number of named sections or a 0
 */
size_t get_nb_config_file_sections(CONFIG_FILE* cfg_file, const char* section_name) {
    ...
    list_foreach(listnode, cfg_file->sections) {
        const CONFIG_FILE_SECTION* section = (const CONFIG_FILE_SECTION*)listnode->ptr;
        if (section_name) {
 ...
 *@return The number of named sections entries or 0
 */
...
char* get_config_section_value(CONFIG_FILE* cfg_file, char* section_name, size_t section_position, char* entry, size_t entry_position) {
    ...
    if (!entries) {
        n_log(LOG_ERR, "section %s not found", section_name);
        return NULL;
    }
