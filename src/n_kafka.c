...
/**
 * @brief return a " (file: ...)" log fragment naming the source file(s) of an event
 * @param event the event to describe
 * @return a never-NULL const C string to append after the opaque pointer in a log line.
 *         When the event carries its source file(s) (event_files_to_delete, set when the
 *         event is produced from a monitored directory) the fragment names them; for
 *         events with no source file (string events, single-file sends, received events)
 *         it is empty so no misleading "(file: ...)" is printed.
 */
static const char* n_kafka_event_fdesc(const N_KAFKA_EVENT* event) {
    static _Thread_local char fragment[2 * PATH_MAX];
    if (event && event->event_files_to_delete && event->event_files_to_delete->data) {
        snprintf(fragment, sizeof(fragment), " (file: %s)", event->event_files_to_delete->data);
        return fragment;
    }
    return "";
}
...
/**
 * @brief create a directory and all its missing parents (mkdir -p)
 * @param path the directory path to create
 * @return TRUE on success (or if it already exists), FALSE on error
 */
static int n_kafka_mkdir_p(const char* path) {
    __n_assert(path, return FALSE);
    ...
    __n_assert(tmp, return FALSE);

    /* strip a trailing slash so the final component is created too */
    if (tmp[len - 1] == '/')
        tmp[len - 1] = '\0';
        ...
        if (*p == '/') {
            *p = '\0';
            if (mkdir(tmp, 0775) != 0 && errno != EEXIST) {
                int error = errno;
                n_log(LOG_ERR, "could not create directory \"%s\": %s", tmp, strerror(error));
                free(tmp);
                return FALSE;
            }
            *p = '/';
...
/**
 * @brief move a single acknowledged event file to the 'sended' directory, preserving its subpath relative to 'to-send'
 * @param kafka the producer handle holding tosend_dir/sended_dir
 * @param filepath the full path of the file to move (as found under tosend_dir)
 * @return TRUE on success, FALSE on error
 */
static int n_kafka_move_file_to_sended(const N_KAFKA* kafka, const char* filepath) {
    __n_assert(kafka, return FALSE);
    __n_assert(kafka->sended_dir, return FALSE);
    __n_assert(filepath, return FALSE);

    /* compute the path relative to the 'to-send' base directory; fall back to
     * the basename when the file does not live under tosend_dir */
    const char* relative = NULL;
    if (kafka->tosend_dir) {
        size_t base_len = strlen(kafka->tosend_dir);
        if (strncmp(filepath, kafka->tosend_dir, base_len) == 0) {
            relative = filepath + base_len;
            while (*relative == '/')
                relative++;
        }
    }
    ...
    if (relative && *relative) {
        nstrprintf(dest, "%s/%s", kafka->sended_dir, relative);
    } else {
        char* tmp = strdup(filepath);
        __n_assert(tmp, return FALSE);
        nstrprintf(dest, "%s/%s", kafka->sended_dir, basename(tmp));
        free(tmp);
    }
    __n_assert(dest, return FALSE);

    /* ensure the destination directory exists */
    char* dest_copy = strdup(_nstr(dest));
    if (dest_copy) {
        n_kafka_mkdir_p(dirname(dest_copy));
        free(dest_copy);
    }

    int ret = TRUE;
    if (rename(filepath, _nstr(dest)) != 0) {
        int error = errno;
        n_log(LOG_ERR, "could not move acknowledged file \"%s\" to \"%s\": %s", filepath, _nstr(dest), strerror(error));
        ret = FALSE;
    } else {
        n_log(LOG_DEBUG, "moved acknowledged file \"%s\" to \"%s\"", filepath, _nstr(dest));
    }
    free_nstr(&dest);
    return ret;
}
...
/**
 * @brief set the retry interval (in seconds) for events stuck in N_KAFKA_EVENT_ERROR
 * @param kafka handler to configure
 * @param error_timeout seconds to wait before re-queuing an errored event; 0 disables retries (default)
 * @return TRUE or FALSE
 */
int n_kafka_set_error_timeout(N_KAFKA* kafka, int32_t error_timeout) {
    __n_assert(kafka, return FALSE);
    if (error_timeout < 0) {
        n_log(LOG_ERR, "n_kafka_set_error_timeout: negative timeout %d rejected", error_timeout);
        return FALSE;
    }
    write_lock(kafka->rwlock);
    kafka->error_timeout = error_timeout;
    unlock(kafka->rwlock);
    return TRUE;
}

/**
 * @brief set the 'to-send' and 'sended' base directories so acknowledged event files are moved instead of deleted
 * @param kafka handler to configure
 * @param tosend_dir base directory the event files are read from; used to compute the subpath to preserve. Can be NULL (then only the basename is preserved)
 * @param sended_dir base directory acknowledged event files are moved into. NULL restores the legacy delete-on-ack behaviour
 * @return TRUE or FALSE
 */
int n_kafka_set_sended_dir(N_KAFKA* kafka, const char* tosend_dir, const char* sended_dir) {
    __n_assert(kafka, return FALSE);
    write_lock(kafka->rwlock);
    FreeNoLog(kafka->tosend_dir);
    FreeNoLog(kafka->sended_dir);
    if (tosend_dir)
        kafka->tosend_dir = strdup(tosend_dir);
    if (sended_dir)
        kafka->sended_dir = strdup(sended_dir);
    unlock(kafka->rwlock);
    return TRUE;
}
 ...
 */
// cppcheck-suppress[constParameterCallback,constParameter] ; callback signature must match rd_kafka typedef
static void n_kafka_delivery_message_callback(rd_kafka_t* rk, const rd_kafka_message_t* rkmessage, void* opaque) {
    ...
    if (rkmessage->err) {
        n_log(LOG_ERR, "message delivery failed for event %p%s: %s", (void*)event, n_kafka_event_fdesc(event), rd_kafka_err2str(rkmessage->err));
        if (!event) {
            n_log(LOG_ERR, "fatal: event is NULL");
            return;
        }
        if (event->parent_table)
            write_lock(event->parent_table->rwlock);
        event->status = N_KAFKA_EVENT_ERROR;
        event->error_time = time(NULL);
        /* adjust counters: event was WAITING_ACK, now it's ERROR */
        if (event->parent_table) {
            event->parent_table->nb_waiting--;
            event->parent_table->nb_error++;
        }
        if (event->parent_table)
        ...
        if (event) {
            ...
            if (event->parent_table)
                write_lock(event->parent_table->rwlock);
            // finalize produce event linked files: move them to the 'sended'
            // directory when one is configured, otherwise delete them (legacy)
            if (event->event_files_to_delete) {
                int move_to_sended = (event->parent_table && event->parent_table->sended_dir) ? 1 : 0;
                ...
                if (files_to_delete) {
                    if (split_count(files_to_delete) > 0) {
                        ...
                        while (files_to_delete[index]) {
                            if (move_to_sended) {
                                n_kafka_move_file_to_sended(event->parent_table, files_to_delete[index]);
                            } else {
            ...
            if (event->parent_table)
                unlock(event->parent_table->rwlock);
            n_log(LOG_INFO, "kafka event %p%s received an ack !", (void*)event, n_kafka_event_fdesc(event));
        } else {
            n_log(LOG_ERR, "fatal: event is NULL");
        }
    }
...
void n_kafka_delete(N_KAFKA* kafka) {
    ...
    /* rd_kafka_topic_destroy must be called before rd_kafka_destroy */
    if (kafka->rd_kafka_topic)
        rd_kafka_topic_destroy(kafka->rd_kafka_topic);
    kafka->rd_kafka_topic = NULL;

    if (kafka->rd_kafka_handle) {
        if (kafka->mode == RD_KAFKA_CONSUMER) {
            /* close the consumer. May already be closed by stop_polling_thread,
             * calling it again is safe (returns an error but does not crash) */
            rd_kafka_consumer_close(kafka->rd_kafka_handle);
            if (kafka->subscription)
...
N_KAFKA* n_kafka_new(int32_t poll_timeout, int32_t poll_interval, size_t errstr_len) {
    ...
    kafka->is_transactional = FALSE;
    kafka->tosend_dir = NULL;
    kafka->sended_dir = NULL;
    ...
    __n_assert(kafka->received_events, n_kafka_delete(kafka); return NULL);
...
N_KAFKA* n_kafka_load_config(char* config_file, int mode) {
    ...
    /* config_string is no longer needed after parsing */
    free_nstr(&config_string);
    if (!json) {
        n_log(LOG_ERR, "unable to parse json from file %s", config_file);
        n_kafka_delete(kafka);
        return NULL;
    }
    ...
    if (mode == RD_KAFKA_PRODUCER) {
        // a configured 'transactional.id' switches the producer to transactional mode.
        // the key itself is already forwarded to rd_kafka_conf_set by the generic
        // loop above (it is a regular librdkafka config key), we only detect it here.
        jstr = cJSON_GetObjectItem(json, "transactional.id");
        if (jstr && jstr->valuestring && jstr->valuestring[0] != '\0') {
            kafka->is_transactional = TRUE;
            n_log(LOG_DEBUG, "kafka producer transactional.id: %s, enabling transactional produces", jstr->valuestring);
        }
        ...
        // initialize the transactional producer (acquires the producer id/epoch).
        // -1 lets librdkafka use 2 * transaction.timeout.ms as the blocking timeout.
        if (kafka->is_transactional) {
            rd_kafka_error_t* txn_err = rd_kafka_init_transactions(kafka->rd_kafka_handle, -1);
            if (txn_err) {
                n_log(LOG_ERR, "failed to init transactions for producer %p, topic %s: %s", kafka->rd_kafka_handle, kafka->topic, rd_kafka_error_string(txn_err));
                rd_kafka_error_destroy(txn_err);
                cJSON_Delete(json);
                n_kafka_delete(kafka);
                return NULL;
            }
            n_log(LOG_DEBUG, "transactions initialized for producer %p, topic %s", kafka->rd_kafka_handle, kafka->topic);
        }
...
int n_kafka_new_headers(N_KAFKA_EVENT* event, size_t count) {
    ...
    if (!event->rd_kafka_headers) {
        ...
        n_log(LOG_ERR, "event headers already allocated for event %p%s", event, n_kafka_event_fdesc(event));
        return FALSE;
    }
    return TRUE;
}
...
int n_kafka_add_header_ex(N_KAFKA_EVENT* event, char* key, size_t key_length, char* value, size_t value_length) {
    ...
    if (key_length < 1 || key_length > SSIZE_MAX) {
        n_log(LOG_ERR, "Invalid key length (%zu) for header in event %p%s", key_length, event, n_kafka_event_fdesc(event));
        return FALSE;
    }

    if (value_length < 1 || value_length > SSIZE_MAX) {
        n_log(LOG_ERR, "Invalid value length (%zu) for key '%s' in event %p%s", value_length, key, event, n_kafka_event_fdesc(event));
        return FALSE;
    }
    ...
    if (err) {
        n_log(LOG_ERR, "Failed to add header [%s:%zu=%s:%zu] to event %p%s: %s",
              key, key_length, value, value_length, event, n_kafka_event_fdesc(event), rd_kafka_err2str(err));
        return FALSE;
    }
...
int n_kafka_produce(N_KAFKA* kafka, N_KAFKA_EVENT* event) {
    ...
    n_log(LOG_DEBUG, "successfully enqueued event %p%s in producer %p waitlist, topic: %s", event, n_kafka_event_fdesc(event), kafka->rd_kafka_handle, kafka->topic);
...
int n_kafka_produce_ex(N_KAFKA* kafka, N_KAFKA_EVENT* event) {
    ...
    event_length = event->event_string->written;
    ...
    if (event->rd_kafka_headers) {
        ...
        if (err) {
            ...
            event->status = N_KAFKA_EVENT_ERROR;
            event->error_time = time(NULL);
            n_log(LOG_ERR, "failed to produce event: %p%s with headers %p, producer: %p, topic: %s, error: %s", event, n_kafka_event_fdesc(event), event->rd_kafka_headers, kafka->rd_kafka_handle, kafka->topic, rd_kafka_err2str(err));
            return FALSE;
        }
    } else {
        if (rd_kafka_produce(kafka->rd_kafka_topic, RD_KAFKA_PARTITION_UA, RD_KAFKA_MSG_F_COPY, event_string, event_length, NULL, 0, event) == -1) {
            ...
            event->status = N_KAFKA_EVENT_ERROR;
            event->error_time = time(NULL);
            n_log(LOG_ERR, "failed to produce event: %p%s, producer: %p, topic: %s, error: %s", event, n_kafka_event_fdesc(event), kafka->rd_kafka_handle, kafka->topic, strerror(error));
            return FALSE;
        }
    }
    // Success to *enqueue* event
    n_log(LOG_DEBUG, "successfully enqueued event %p%s in local producer %p : %s", event, n_kafka_event_fdesc(event), kafka->rd_kafka_handle, kafka->topic);
...
N_KAFKA_EVENT* n_kafka_new_event_from_char(const char* string, size_t written, int schema_id) {
    ...
    __n_assert(event->event_string->data, free(event->event_string); free(event); return NULL);
...
N_KAFKA_EVENT* n_kafka_new_event_from_file(char* filename, int schema_id) {
    ...
    N_KAFKA_EVENT* event = n_kafka_new_event_from_string(from, schema_id);
    free_nstr(&from);
...
int n_kafka_poll(N_KAFKA* kafka) {
    ...
    int event_consumption_enabled;
    int event_production_enabled;
    ...
    if (kafka->mode == RD_KAFKA_PRODUCER) {
        if (event_production_enabled) {
            ...
            (void)nb_events;
            /* transactional mode: a kafka transaction is opened lazily before the
             * first queued event is produced in this cycle, and committed after the
             * locked pass below. commit/abort must run outside the rwlock because
             * they synchronously fire the delivery callbacks which take that lock. */
            int txn_active = 0;
            int txn_begin_failed = 0;
            write_lock(kafka->rwlock);
            ...
            while (node) {
                ...
                if (event->status == N_KAFKA_EVENT_OK) {
                    kafka->nb_waiting--;
                    n_log(LOG_DEBUG, "removing event OK %p%s", (void*)event, n_kafka_event_fdesc(event));
                    ...
                    /* in transactional mode, open a transaction before the first produce */
                    if (kafka->is_transactional && !txn_active && !txn_begin_failed) {
                        rd_kafka_error_t* txn_err = rd_kafka_begin_transaction(kafka->rd_kafka_handle);
                        if (txn_err) {
                            n_log(LOG_ERR, "could not begin transaction on producer %p, topic %s: %s%s", kafka->rd_kafka_handle, kafka->topic, rd_kafka_error_string(txn_err), rd_kafka_error_is_fatal(txn_err) ? " (FATAL)" : "");
                            rd_kafka_error_destroy(txn_err);
                            /* leave the queued events untouched, retry on the next poll */
                            txn_begin_failed = 1;
                        } else {
                            txn_active = 1;
                        }
                    }
                    /* if a transaction was required but could not be opened, skip producing this cycle */
                    if (kafka->is_transactional && !txn_active) {
                        node = node->next;
                        continue;
                    }
                    if (n_kafka_produce_ex(kafka, event) == FALSE) {
                        /* produce_ex already set status to ERROR,
                         * adjust counters: QUEUED -> ERROR */
                        kafka->nb_queued--;
                        kafka->nb_error++;
                    } else {
                        kafka->nb_waiting++;
                    ...
                    /* counters already adjusted at transition time
                     * (in produce_ex failure above or in delivery callback) */
                    if (kafka->error_timeout > 0 &&
                        (time(NULL) - event->error_time) >= (time_t)kafka->error_timeout) {
                        n_log(LOG_INFO, "retrying errored event %p%s after %d s", (void*)event, n_kafka_event_fdesc(event), kafka->error_timeout);
                        event->status = N_KAFKA_EVENT_QUEUED;
                        event->error_time = 0;
                        kafka->nb_error--;
                        kafka->nb_queued++;
            ...
            /* close the transaction outside the lock: commit_transaction (and
             * abort_transaction) drive the delivery callbacks that, on success,
             * move the produced files to 'sended' and, on failure, flip events
             * back to ERROR. Both callbacks take the rwlock themselves.
             * Retry the commit on retriable errors; on any other failure (or once
             * retries are exhausted) abort to bring the producer back to a clean
             * state: the purged messages are reported to the delivery callback as
             * errors so their files stay in 'to-send' and are produced again later. */
            if (txn_active) {
                int committed = 0;
                int attempts = 0;
                while (!committed && attempts < 5) {
                    attempts++;
                    rd_kafka_error_t* commit_err = rd_kafka_commit_transaction(kafka->rd_kafka_handle, -1);
                    if (!commit_err) {
                        committed = 1;
                        break;
                    }
                    n_log(LOG_ERR, "could not commit transaction (attempt %d) on producer %p, topic %s: %s", attempts, kafka->rd_kafka_handle, kafka->topic, rd_kafka_error_string(commit_err));
                    int retriable = rd_kafka_error_is_retriable(commit_err);
                    int fatal = rd_kafka_error_is_fatal(commit_err);
                    rd_kafka_error_destroy(commit_err);
                    if (retriable)
                        continue;  // resume the in-flight commit
                    if (fatal)
                        n_log(LOG_ERR, "FATAL transaction error on producer %p, topic %s, producer is no longer usable", kafka->rd_kafka_handle, kafka->topic);
                    break;  // abort-required, fatal or other non-retriable error
                }
                if (!committed) {
                    n_log(LOG_ERR, "aborting transaction on producer %p, topic %s", kafka->rd_kafka_handle, kafka->topic);
                    rd_kafka_error_t* abort_err = rd_kafka_abort_transaction(kafka->rd_kafka_handle, -1);
                    if (abort_err) {
                        n_log(LOG_ERR, "could not abort transaction on producer %p, topic %s: %s", kafka->rd_kafka_handle, kafka->topic, rd_kafka_error_string(abort_err));
                        rd_kafka_error_destroy(abort_err);
                    }
                }
            }
        } else {
            uint64_t sleep_us = (kafka->poll_interval > 0) ? ((uint64_t)(uint32_t)kafka->poll_interval * 1000ULL) : 0ULL;
            usleep((unsigned int)(sleep_us > (uint64_t)UINT_MAX ? (uint64_t)UINT_MAX : sleep_us));  // production is disabled, sleep instead of producing
        }
    } else if (kafka->mode == RD_KAFKA_CONSUMER) {
        if (event_consumption_enabled) {
            rd_kafka_message_t* rkm = NULL;
            while (event_consumption_enabled && (rkm = rd_kafka_consumer_poll(kafka->rd_kafka_handle, kafka->poll_interval))) {
                read_lock(kafka->rwlock);
                event_consumption_enabled = kafka->event_consumption_enabled;
                unlock(kafka->rwlock);
                if (rkm->err) {
                    ...
                    uint64_t sleep_us = (kafka->poll_interval > 0) ? ((uint64_t)(uint32_t)kafka->poll_interval * 1000ULL) : 0ULL;
                    usleep((unsigned int)(sleep_us > (uint64_t)UINT_MAX ? (uint64_t)UINT_MAX : sleep_us));
                    return FALSE;
                }
            ...
            uint64_t sleep_us = (kafka->poll_interval > 0) ? ((uint64_t)(uint32_t)kafka->poll_interval * 1000ULL) : 0ULL;
            usleep((unsigned int)(sleep_us > (uint64_t)UINT_MAX ? (uint64_t)UINT_MAX : sleep_us));  // consumption is disabled, sleep instead of consuming
        }
    }
...
void* n_kafka_polling_thread(void* ptr) {
    ...
    int64_t remaining_time = (int64_t)kafka->poll_timeout * 1000;
    while (status == 1) {
...
int n_kafka_stop_polling_thread(N_KAFKA* kafka) {
    ...
    if (kafka->mode == RD_KAFKA_CONSUMER) {
        n_kafka_disable_event_consumption(kafka);
    }
    ...
    if (kafka->rd_kafka_handle && kafka->mode == RD_KAFKA_CONSUMER) {
        rd_kafka_consumer_close(kafka->rd_kafka_handle);
    }
    ...
    int rc = pthread_timedjoin_np(kafka->polling_thread, NULL, &ts);
    if (rc != 0) {
        n_log(LOG_ERR, "polling thread did not stop in 10s, force stop !");
        return FALSE;
    }
 ...
 * @param kafka the N_KAFKA instance
 * @return TRUE or FALSE
 */
...
int n_kafka_dump_unprocessed(N_KAFKA* kafka, char* directory) {
    ...
    /* use a plain struct to alias event data without allocating a data buffer
     * that would leak when overwritten */
    N_STR* dumpstr = NULL;
    Malloc(dumpstr, N_STR, 1);
    __n_assert(dumpstr, unlock(kafka->rwlock); return FALSE);
    list_foreach(node, kafka->events_to_send) {
        ...
        if (event->status != N_KAFKA_EVENT_OK) {
            ...
            n_log(LOG_DEBUG, "Dumping unprocessed event %p%s to %s", (void*)event, n_kafka_event_fdesc(event), _nstr(filename));
            // dump event here: alias the data pointer, do not own it
            dumpstr->data = event->event_string->data + offset;
            dumpstr->written = event->event_string->written - offset;
            dumpstr->length = event->event_string->length - offset;
            nstr_to_file(dumpstr, _nstr(filename));
            free_nstr(&filename);
        }
    }
    unlock(kafka->rwlock);
    /* clear aliased pointer before freeing the wrapper struct */
    dumpstr->data = NULL;
    free_nstr(&dumpstr);
    return TRUE;
...
int n_kafka_load_unprocessed(N_KAFKA* kafka, const char* directory) {
    ...
    write_lock(kafka->rwlock);
    /* load events from filename using base64decode( filename ) split( result , '+' )
       to get brokersname+topic to check against what's saved with the event
       and what's inside kafka's handle conf */
    unlock(kafka->rwlock);
