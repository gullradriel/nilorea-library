...
N_KAFKA* n_kafka_load_config(char* config_file, int mode) {
    ...
    for (jsonIndex = 0; jsonIndex < cJSON_GetArraySize(json); jsonIndex++) {
        ...
        if (entry->string[0] != '-') {
            ...
            if (strcmp("topic", entry->string) != 0 && strcmp("topics", entry->string) != 0 && strcmp("value.schema.id", entry->string) != 0 && strcmp("value.schema.type", entry->string) != 0 && strcmp("poll.interval", entry->string) != 0 && strcmp("poll.timeout", entry->string) != 0 && strcmp("group.id.autogen", entry->string) != 0) {
