...
/*!\fn int resize_nstr( N_STR *nstr , size_t size )
...
/*!\fn N_STR *nstrprintf_ex(N_STR *nstr_var, const char *format, ...)
...
/*!\fn N_STR *nstrprintf_cat(N_STR **nstr_var, const char *format, ...)
...
N_STR* nstrprintf_cat_ex(N_STR** nstr_var, const char* format, ...) {
    ...
    if (!(*nstr_var)) {
        // If the N_STR is not initialized, delegate to nstrprintf.
        va_start(args, format);
        va_copy(args_copy, args);
        (*nstr_var) = nstrprintf((*nstr_var), format, args_copy);
        va_end(args_copy);
        va_end(args);
        return (*nstr_var);
    }
    ...
    vsnprintf((*nstr_var)->data + (*nstr_var)->written, needed, format, args_copy);
    (*nstr_var)->written += (size_t)needed_size;
