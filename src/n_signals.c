...
/**
 *@file n_signals.c
 *@brief Signals general handling with stack printing
...
#else
#ifdef __windows__
    snprintf(addr2line_cmd, sizeof(addr2line_cmd), "addr2line -f -p -e ./%s %p", program_name, addr);
 ...
 *@param context the context to unroll
 */
// cppcheck-suppress constParameterCallback ; callback signature must match system typedef
...
void set_signal_handler(const char* progname) {
    ...
    /* cppcheck-suppress ConfigurationNotChecked ; RLIMIT_STACK is a system macro, value not relevant to static analysis */
