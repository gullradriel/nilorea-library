...
/**@file n_user.c
 *@brief Network user handling functions
...
int userlist_set_position_behavior(N_USERLIST* ulist, int id, int nb_rec_pos, int only_last_pos) {
    ...
    /* nb_rec_pos must be at least 1 */
    if (nb_rec_pos < 1) return FALSE;
    /* only_last_pos is a boolean: 0 or 1 */
    if (only_last_pos < 0 || only_last_pos > 1) return FALSE;

    read_lock(ulist->user_rwbolt);
    if (id < 0 || id >= ulist->max) {
        unlock(ulist->user_rwbolt);
        return FALSE;
    }
    /* user slot must be active */
    ...
    if (only_last_pos) {
        if (Realloc(ulist->list[id].last_positions, VECTOR3D, 1) == FALSE) {
            n_log(LOG_ERR, "could not resize to only_last_pos VECTOR3D");
            unlock(ulist->user_rwbolt);
            return FALSE;
        }
    } else {
        if (Realloc(ulist->list[id].last_positions, VECTOR3D, (size_t)nb_rec_pos) == FALSE) {
            n_log(LOG_ERR, "could not resize to %d VECTOR3D", nb_rec_pos);
            unlock(ulist->user_rwbolt);
            return FALSE;
        }
        /* nb_rec_pos is already set correctly above; do not overwrite it */
    }
    unlock(ulist->user_rwbolt);
...
/**
 *@brief add a network message to specified user 'id'
...
/**
 *@brief send all waiting messages for each user of the list
 *@param ulist targeted N_USERLIST *list
 *@return TRUE or FALSE
 */
