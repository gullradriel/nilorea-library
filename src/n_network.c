...
int netw_set_blocking(NETWORK* netw, unsigned long int is_blocking) {
    ...
    if (fcntl(netw->link.sock, F_SETFL, is_blocking ? flags & ~O_NONBLOCK : flags | O_NONBLOCK) == -1) {
        ...
        _netw_capture_error(netw, "couldn't set blocking mode %lu on %d: %s", is_blocking, netw->link.sock, _str(errmsg));
        n_log(LOG_ERR, "couldn't set blocking mode %lu on %d: %s", is_blocking, netw->link.sock, _str(errmsg));
        FreeNoLog(errmsg);
        return FALSE;
    }
#else
...
/**
 *@brief print the queued OpenSSL errors for a given socket
 *@param socket the socket to report errors for
 *@details Drains the per-thread OpenSSL error queue, logging each entry at
 *LOG_ERR, except the benign "unexpected eof while reading" (a peer closing the
 *connection without a TLS close_notify, common from browsers) which is logged at
 *LOG_DEBUG so it does not masquerade as a real TLS failure.
 */
void netw_ssl_print_errors(SOCKET socket) {
    ...
    while ((error = ERR_get_error())) {
        ...
        /* A peer that closes the TCP connection without a TLS close_notify makes
           OpenSSL 3.x queue "unexpected eof while reading". Clients (web browsers
           especially, on speculative/preconnect sockets) do this routinely, so it
           is expected disconnect noise rather than an error: log it at DEBUG so it
           does not drown a real TLS failure, which keeps its own reason code and
           LOG_ERR level. */
        if (ERR_GET_REASON(error) == SSL_R_UNEXPECTED_EOF_WHILE_READING)
            level = LOG_DEBUG;
#endif
        n_log(level, "socket %d: %s", socket, ERR_reason_error_string(error));
    }
}
...
int netw_set_crypto(NETWORK* netw, char* key, char* certificate) {
    ...
    if (key && certificate) {
        ...
        /* 2026-07-18 — send ZERO TLS 1.3 session tickets. The threaded
         * client engine runs SSL_read (recv thread) and SSL_write (send
         * thread) on the same SSL*; OpenSSL does not support concurrent
         * use, and the one window where the read path MUTATES shared
         * session state is post-handshake NewSessionTicket processing —
         * which lands at the exact moment the client fires its first
         * application write (HELLO). On low-RTT links (loopback / LAN)
         * the overlap corrupts the client's outbound record stream and
         * the server dies with "decryption failed or bad record mac" on
         * its FIRST read (torn records can also sit unreadable for
         * seconds until more bytes complete them). Nothing here uses TLS
         * session resumption, so tickets are pure race surface — drop
         * them at the source. Higher-RTT links (the 8 ms VPS) rarely hit
         * the window, which is why this only surfaced on local servers. */
        SSL_CTX_set_num_tickets(netw->ctx, 0);
#endif
...
#if OPENSSL_VERSION_NUMBER >= 0x10101000L
    /* 2026-07-18 — zero TLS 1.3 session tickets; see the twin comment in
     * netw_set_crypto: post-handshake NewSessionTicket processing in the
     * client's recv thread races the send thread's first SSL_write on the
     * shared SSL* ("bad record mac" at connect on low-RTT links). No
     * consumer uses session resumption. */
    SSL_CTX_set_num_tickets(netw->ctx, 0);
#endif
...
/* Install an in-memory certificate and key on a single SSL connection. Shared
 * by netw_set_crypto_pem_ctx and the SNI accept callback. Returns 0 on success,
 * -1 on error. */
static int _ssl_use_pem(SSL* ssl, const char* key_pem, const char* cert_pem) {
    ...
    if (SSL_use_certificate(ssl, cert) == 1) {
        ...
        if (kbio) {
            ...
            BIO_free(kbio);
            if (pkey) {
                if (SSL_use_PrivateKey(ssl, pkey) == 1 && SSL_check_private_key(ssl) == 1)
                    rc = 0;
                EVP_PKEY_free(pkey);
            }
        }
...
/**
 * @brief get the SNI server_name the peer requested in its ClientHello
 * @param netw the accepted NETWORK
 * @return the requested host name, or NULL if none / not a TLS connection
 */
const char* netw_ssl_get_sni(NETWORK* netw) {
    __n_assert(netw, return NULL);
    if (!netw->ssl)
        return NULL;
    return SSL_get_servername(netw->ssl, TLSEXT_NAMETYPE_host_name);
}

/**
 * @brief install an in-memory cert/key on a single accepted connection's SSL
 * @param netw the accepted NETWORK (must already hold an SSL handle)
 * @param key_pem the private key PEM
 * @param cert_pem the certificate PEM
 * @return TRUE or FALSE
 */
int netw_set_crypto_pem_ctx(NETWORK* netw, const N_STR* key_pem, const N_STR* cert_pem) {
    __n_assert(netw, return FALSE);
    __n_assert(netw->ssl, return FALSE);
    __n_assert(key_pem && key_pem->data, return FALSE);
    __n_assert(cert_pem && cert_pem->data, return FALSE);
    if (_ssl_use_pem(netw->ssl, key_pem->data, cert_pem->data) != 0) {
        netw_ssl_print_errors(netw->link.sock);
        return FALSE;
    }
    return TRUE;
...
/* Arg handed (by stack pointer) to the servername callback for the duration of
 * a single netw_accept_ssl_with_sni_cb call. */
typedef struct NETW_SNI_ARG {
    netw_sni_pick_cb pick;
    void* user_data;
} NETW_SNI_ARG;

/* OpenSSL servername callback: fires during SSL_accept once the ClientHello is
 * parsed. Picks a per-host certificate via the caller's pick() and installs it
 * on the current SSL so the handshake completes with a host-matching leaf. */
static int _netw_sni_servername_cb(SSL* ssl, int* al, void* arg) {
    NETW_SNI_ARG* a = (NETW_SNI_ARG*)arg;
    const char* sni;
    N_STR* cert_pem = NULL;
    N_STR* key_pem = NULL;
    int ok;
    ...
    if (a->pick(sni, &cert_pem, &key_pem, a->user_data) != 0 || !cert_pem || !key_pem) {
        if (cert_pem)
            free_nstr(&cert_pem);
        if (key_pem)
            free_nstr(&key_pem);
    ...
    ok = (_ssl_use_pem(ssl, key_pem->data, cert_pem->data) == 0);
    free_nstr(&cert_pem);
    free_nstr(&key_pem);
    return ok ? SSL_TLSEXT_ERR_OK : SSL_TLSEXT_ERR_ALERT_FATAL;
}

/**
 * @brief upgrade an already-accepted plaintext connection to a TLS server,
 *        selecting the certificate per client SNI
 *
 * Sets up a per-connection SSL server context with no default certificate plus
 * an OpenSSL servername callback, then performs SSL_accept on the socket. The
 * callback runs synchronously inside SSL_accept and asks pick() for a
 * certificate matching the requested host, so the handshake completes with a
 * host-specific leaf. Use this when the socket was accepted in the clear first
 * (for example after a proxy CONNECT) and must then become a TLS server.
 *
 * @param netw an already-accepted plaintext NETWORK
 * @param pick callback that supplies a cert/key for a given SNI host
 * @param user_data opaque pointer passed to pick
 * @return TRUE or FALSE
 */
int netw_ssl_server_handshake(NETWORK* netw, netw_sni_pick_cb pick, void* user_data) {
    NETW_SNI_ARG arg;
    __n_assert(netw, return FALSE);
    __n_assert(pick, return FALSE);
    ...
    SSL_CTX_set_mode(netw->ctx, SSL_MODE_ENABLE_PARTIAL_WRITE | SSL_MODE_ACCEPT_MOVING_WRITE_BUFFER);
    SSL_CTX_set_tlsext_servername_callback(netw->ctx, _netw_sni_servername_cb);

    /* The callback runs synchronously inside the blocking SSL_accept below, so a
     * stack-scoped arg is valid for its whole lifetime and needs no heap/free. */
    arg.pick = pick;
    arg.user_data = user_data;
    SSL_CTX_set_tlsext_servername_arg(netw->ctx, &arg);
...
/**
 * @brief accept a TLS connection, selecting the server certificate per client SNI
 *
 * Accepts a plaintext connection from @p listen, then upgrades it to a TLS
 * server via netw_ssl_server_handshake(), so the pick callback supplies a leaf
 * matching the client's requested host during the handshake.
 *
 * @param listen the listening NETWORK from netw_make_listening
 * @param pick callback that supplies a cert/key for a given SNI host
 * @param user_data opaque pointer passed to pick
 * @return the accepted NETWORK, or NULL on error
 */
NETWORK* netw_accept_ssl_with_sni_cb(NETWORK* listen, netw_sni_pick_cb pick, void* user_data) {
    NETWORK* client;
    __n_assert(listen, return NULL);
    __n_assert(pick, return NULL);
    client = netw_accept_from(listen);
    if (!client)
        return NULL;
    if (netw_ssl_server_handshake(client, pick, user_data) != TRUE) {
        netw_close(&client);
        return NULL;
    }
    return client;
...
/**
 * @brief evaluate the peer certificate of a completed TLS client connection
 *
 * Verifies the peer certificate chain against the system default trust store
 * and checks that expected_host matches the certificate, independently of the
 * verify mode the handshake ran with. This lets a connection complete with
 * verification disabled (for example an intercepting proxy that must see the
 * traffic regardless) while still reporting whether the real peer certificate
 * would have been trusted.
 *
 * @param netw a NETWORK with a completed client handshake (netw->ssl set)
 * @param expected_host hostname to match against the certificate, or NULL to skip
 * @param errbuf optional buffer that receives a short reason on failure
 * @param errsz size of errbuf
 * @return TRUE when the chain is trusted and the host matches, FALSE otherwise
 */
int netw_ssl_get_verify_result(NETWORK* netw, const char* expected_host, char* errbuf, size_t errsz) {
    ...
    chain = SSL_get_peer_cert_chain(netw->ssl);
    ...
    if (X509_STORE_CTX_init(store_ctx, store, cert, chain) != 1) {
        if (errbuf && errsz > 0)
            snprintf(errbuf, errsz, "verify init failed");
        goto cleanup;
    }
    if (X509_verify_cert(store_ctx) == 1) {
        chain_ok = 1;
    } else {
        int verr = X509_STORE_CTX_get_error(store_ctx);
        if (errbuf && errsz > 0)
            snprintf(errbuf, errsz, "%s", X509_verify_cert_error_string(verr));
    }
    if (expected_host && expected_host[0]) {
        if (X509_check_host(cert, expected_host, 0, 0, NULL) != 1) {
            host_ok = 0;
            if (errbuf && errsz > 0 && chain_ok)
                snprintf(errbuf, errsz, "hostname mismatch for %s", expected_host);
        }
    }
    result = (chain_ok && host_ok) ? TRUE : FALSE;
...
/*! Poll interval (milliseconds) at which an in-progress connect wakes to check
 *  the registered connect-abort callback (see netw_set_connect_abort_cb). */
#define NETW_CONNECT_ABORT_POLL_MS 100

/*! Optional connect-abort callback, or NULL. See netw_set_connect_abort_cb. */
...
static int _netw_timed_connect(NETWORK* netw, SOCKET sock, struct addrinfo* rp, int connect_timeout_ms) {
    ...
    /* select() blocks the thread in the kernel (no busy-wait). If a signal
       ...
       When a connect-abort callback is registered the per-select wait is
       capped to NETW_CONNECT_ABORT_POLL_MS so the loop wakes periodically to
       poll it, letting a caller interrupt a connect to an unresponsive host
       without waiting out the whole connect timeout. */
    N_TIME timer;
    start_HiTimer(&timer);
    time_t remaining_us = (time_t)connect_timeout_ms * 1000;
    time_t poll_us = _netw_connect_abort_cb ? (time_t)NETW_CONNECT_ABORT_POLL_MS * 1000 : 0;
    int connected = 0;
    int aborted = 0;
    for (;;) {
        if (_netw_connect_abort_cb && _netw_connect_abort_cb(_netw_connect_abort_ctx)) {
            aborted = 1;
            break;
        }
        ...
        FD_ZERO(&eset);
        FD_SET(sock, &eset);
        time_t wait_us = remaining_us;
        if (poll_us > 0 && wait_us > poll_us)
            wait_us = poll_us;
        struct timeval tv;
        tv.tv_sec = (long)(wait_us / 1000000);
        tv.tv_usec = (long)(wait_us % 1000000);
        ...
        if (sel == 0) {
            /* the select() wait expired: subtract it from the remaining budget.
               If this was only a poll slice (an abort callback is registered) and
               time is left, loop to re-check the callback; otherwise the connect
               timeout has been reached. */
            remaining_us -= get_usec(&timer);
            if (remaining_us <= 0)
                break; /* connect timed out */
            continue;
        }
        /* sel < 0 */
#ifdef __windows__
...
/**
 *@brief Register a process-wide callback polled while a connect is in progress.
 *
 * When set, netw_connect_ex_to (and every _to / SSL connect variant built on it)
 * wakes every NETW_CONNECT_ABORT_POLL_MS during the connect wait and calls @p cb;
 * if it returns non-zero the connect is abandoned at once (the connect call
 * returns FALSE) instead of blocking until connect_timeout_ms. This lets an
 * application interrupt a connect to an unresponsive host (a dropped SYN) the way
 * shutdown() already interrupts a blocking read on a connected socket. The
 * callback must be quick and non-blocking. Pass @p cb = NULL to remove it. The
 * pointer is a single process-global; set it once at startup before concurrent
 * connects (it is read without a lock).
 *
 *@param cb Callback returning non-zero to abort the in-progress connect, or NULL.
 *@param ctx Opaque pointer handed back to @p cb on each poll.
 */
void netw_set_connect_abort_cb(int (*cb)(void* ctx), void* ctx) {
    _netw_connect_abort_cb = cb;
    _netw_connect_abort_ctx = ctx;
...
int netw_ssl_connect_client_to(NETWORK** netw, char* host, char* port, int ip_version, int connect_timeout_ms) {
    ...
    if (netw_ssl_start_client(*netw) == FALSE) {
        netw_close(netw);
        return FALSE;
    }
    return TRUE;
...
/**
 * @brief Set up a client SSL_CTX on an already-connected NETWORK.
 *
 * Performs the SSL-context half of netw_ssl_connect_client_to on a NETWORK
 * whose TCP connection is already established (for example one returned by
 * netw_adopt_client_fd after an HTTP CONNECT proxy tunnel): it creates an
 * SSL_CTX with TLS_client_method, loads the default system verify paths, and
 * switches the NETWORK send/recv vtable to the SSL variants. It does NOT
 * perform the handshake. Call netw_ssl_set_ca / netw_ssl_set_verify to
 * configure verification, then netw_ssl_do_handshake to complete the
 * connection. On failure the NETWORK is left untouched (the caller owns it and
 * is responsible for netw_close).
 *
 * @param netw connected NETWORK to promote to SSL. Must not be NULL.
 * @return TRUE or FALSE
 */
...
int netw_add_msg(NETWORK* netw, N_STR* msg) {
    ...
    if (msg->length == 0) {
        _netw_capture_error(netw, "Empty messages are not supported. msg(%p)->length=%zu", msg, msg->length);
        n_log(LOG_ERR, "Empty messages are not supported. msg(%p)->length=%zu", msg, msg->length);
        return FALSE;
    }
...
ssize_t send_data(void* netw, char* buf, uint32_t n) {
    ...
    while (bcount < n) /* loop until all sent */
    {
        ...
        if (bs > 0) {
            ...
            /* real network error */
            errmsg = netstrerror(error);
            _netw_capture_error((NETWORK*)netw, "Socket %d send error: %zd, %s", s, bs, _str(errmsg));
            n_log(LOG_ERR, "Socket %d send error: %zd, %s", s, bs, _str(errmsg));
            FreeNoLog(errmsg);
            return NETW_SOCKET_ERROR;
        } else if (bs == -2) {
...
ssize_t recv_data(void* netw, char* buf, uint32_t n) {
    ...
    while (bcount < n) /* loop until all received */
    {
        ...
        if (br > 0) {
            ...
            /* real network error */
            errmsg = netstrerror(error);
            _netw_capture_error((NETWORK*)netw, "Socket %d recv error: %zd, %s", s, br, _str(errmsg));
            n_log(LOG_ERR, "Socket %d recv error: %zd, %s", s, br, _str(errmsg));
            FreeNoLog(errmsg);
            return NETW_SOCKET_ERROR;
        } else if (br == -2) {
            /* EINTR/WOULDBLOCK retries exhausted */
        ...
        if (status > 0) {
            ...
            switch (ssl_error) {
                ...
                case SSL_ERROR_SSL:
                    ...
                    _netw_capture_error((NETWORK*)netw, "socket %d SSL_write returned %zu, error: %s", s, bs, ERR_reason_error_string(ERR_get_error()));
                    n_log(LOG_ERR, "socket %d SSL_write returned %zu, error: %s", s, bs, ERR_reason_error_string(ERR_get_error()));
                    break;
                default:
                    // Other errors
                    _netw_capture_error((NETWORK*)netw, "socket %d SSL_write returned %zu, errno: %s", s, bs, _str(errmsg));
                    n_log(LOG_ERR, "socket %d SSL_write returned %zu, errno: %s", s, bs, _str(errmsg));
                    break;
            }
            FreeNoLog(errmsg);
            return NETW_SOCKET_ERROR;
...
int n_http_status_is_informational(int status_code) {
    return n_http_status_class(status_code) == 1;
}

int n_http_status_is_success(int status_code) {
    return n_http_status_class(status_code) == 2;
}

int n_http_status_is_redirect(int status_code) {
    return n_http_status_class(status_code) == 3;
}

int n_http_status_is_client_error(int status_code) {
    return n_http_status_class(status_code) == 4;
}

int n_http_status_is_server_error(int status_code) {
    return n_http_status_class(status_code) == 5;
}

/* Inflate src/len with the given zlib window bits into a fresh N_STR.
 * window_bits: 15+16 for gzip framing, -15 for raw deflate, 15 for zlib-wrapped.
 * Returns 0 on success (*out set), -1 on error. */
static int n_http_inflate(const unsigned char* src, size_t len, int window_bits, N_STR** out) {
    z_stream strm;
    size_t alloc, total = 0;
    char* buf;
    int ret;
    *out = NULL;
    ...
    do {
        ...
        strm.avail_out = (uInt)(alloc - total - 1);
        ...
        total = alloc - 1 - strm.avail_out;
    } while (ret != Z_STREAM_END);
    inflateEnd(&strm);
    buf[total] = '\0';
    {
        N_STR* r = NULL;
        char_to_nstr_ex(buf, (NSTRBYTE)total, &r);
        free(buf);
        if (!r)
            return -1;
        *out = r;
...
int n_http_decompress_body(const N_STR* body, const char* encoding, N_STR** out) {
    ...
    *out = NULL;
    ...
    if (encoding) {
        size_t i;
        for (i = 0; i < sizeof(enc) - 1 && encoding[i]; i++)
            enc[i] = (char)tolower((unsigned char)encoding[i]);
        enc[i] = '\0';
    }
    if (strstr(enc, "gzip"))
        is_gzip = 1;
    else if (strstr(enc, "deflate"))
        is_deflate = 1;

    /* identity / unknown / empty body: pass a copy through unchanged */
    if ((!is_gzip && !is_deflate) || !body->data || body->written == 0) {
        N_STR* copy = nstrdup((N_STR*)body);
        if (!copy)
            return -1;
        *out = copy;
        return 0;
    ...
    if (is_gzip)
        return n_http_inflate((const unsigned char*)body->data, body->written, 15 + 16, out);
    /* "deflate" is sent both raw (-15) and zlib-wrapped (15); try raw, then wrapped */
    if (n_http_inflate((const unsigned char*)body->data, body->written, -15, out) == 0)
        return 0;
    return n_http_inflate((const unsigned char*)body->data, body->written, 15, out);
}

/* Binary substring search: first offset of needle in hay, or -1. */
static long mp_find(const unsigned char* hay, size_t hlen, const char* needle, size_t nlen) {
    size_t i;
    if (nlen == 0 || nlen > hlen)
        return -1;
    for (i = 0; i + nlen <= hlen; i++) {
        if (hay[i] == (unsigned char)needle[0] && memcmp(hay + i, needle, nlen) == 0)
            return (long)i;
    }
    return -1;
}

/* Duplicate len bytes of s into a fresh NUL-terminated C string. */
...
int n_http_multipart_boundary(const char* content_type, char* out, size_t outsz) {
    const char* p;
    size_t i;
    if (!content_type || !out || outsz == 0)
        return -1;
    /* case-insensitive search for "boundary=" */
...
/* Free an N_HTTP_MULTIPART_PART stored in a LIST. */
static void mp_part_free(void* ptr) {
    N_HTTP_MULTIPART_PART* mp = (N_HTTP_MULTIPART_PART*)ptr;
    if (!mp)
        return;
    FreeNoLog(mp->name);
    FreeNoLog(mp->filename);
    FreeNoLog(mp->content_type);
    if (mp->body)
        free_nstr(&mp->body);
    Free(mp);
}
...
/* Within a header block, find the value of the named header (case-insensitive),
 * returning a pointer to the value start and its length (trailing CRLF trimmed). */
static const char* mp_header_value(const char* block, size_t blen, const char* name, size_t* vlen) {
    ...
    *vlen = 0;
    while (i < blen) {
        size_t line_end = i;
        while (line_end < blen && block[line_end] != '\n')
            line_end++;
        /* line spans [i, line_end) (may include a trailing '\r') */
        if (line_end - i > nlen + 1 && strncasecmp(block + i, name, nlen) == 0 && block[i + nlen] == ':') {
            const char* v = block + i + nlen + 1;
            size_t vl = line_end - (i + nlen + 1);
            while (vl > 0 && (*v == ' ' || *v == '\t')) {
                v++;
                vl--;
            }
            while (vl > 0 && (v[vl - 1] == '\r' || v[vl - 1] == ' ' || v[vl - 1] == '\t'))
                vl--;
            *vlen = vl;
            return v;
        }
...
/* Extract a quoted (or token) parameter value from a Content-Disposition value,
 * e.g. param "name" from `form-data; name="x"; filename="y"`. */
static char* mp_disp_param(const char* disp, size_t dlen, const char* param) {
    char key[32];
    size_t klen, i;
    snprintf(key, sizeof(key), "%s=", param);
    klen = strlen(key);
    for (i = 0; i + klen <= dlen; i++) {
        if (strncasecmp(disp + i, key, klen) == 0) {
            const char* v = disp + i + klen;
            size_t avail = dlen - (i + klen);
            size_t j = 0;
            if (avail > 0 && *v == '"') {
                v++;
                avail--;
                ...
                while (j < avail && v[j] != ';' && v[j] != ' ')
                    j++;
            }
            return mp_strndup(v, j);
        }
    }
    return NULL;
}
...
LIST* n_http_parse_multipart(const N_STR* body, const char* boundary) {
    ...
    size_t len, dlen, cdlen, cursor;
    long first;
    LIST* parts;

    if (!body || !body->data || !boundary || !boundary[0])
        return NULL;
    if (strlen(boundary) + 2 >= sizeof(dash))
        return NULL;
    snprintf(dash, sizeof(dash), "--%s", boundary);
    snprintf(crlf_dash, sizeof(crlf_dash), "\r\n--%s", boundary);
    dlen = strlen(dash);
    cdlen = strlen(crlf_dash);

    parts = new_generic_list(0);
    if (!parts)
        return NULL;

    data = (const unsigned char*)body->data;
    len = body->written;
    first = mp_find(data, len, dash, dlen);
    if (first < 0)
        return parts; /* no boundary present: empty result */
    cursor = (size_t)first + dlen;

    for (;;) {
        long rel;
        size_t part_start, part_end;
        size_t hvl;
        N_HTTP_MULTIPART_PART* mp;
        /* closing delimiter "--boundary--" */
        if (cursor + 2 <= len && data[cursor] == '-' && data[cursor + 1] == '-')
            break;
        /* skip the CRLF that follows the delimiter */
        if (cursor + 2 <= len && data[cursor] == '\r' && data[cursor + 1] == '\n')
            cursor += 2;
        else if (cursor < len && data[cursor] == '\n')
            cursor += 1;
        part_start = cursor;
        rel = mp_find(data + part_start, len - part_start, crlf_dash, cdlen);
        if (rel < 0)
            break; /* malformed: no closing delimiter */
        part_end = part_start + (size_t)rel;

        Malloc(mp, N_HTTP_MULTIPART_PART, 1);
        if (!mp)
            break;
        mp->name = NULL;
        mp->filename = NULL;
        ...
        mp->body = NULL;
        /* split the part into its header block and body at the blank line */
        {
            long s = mp_find(data + part_start, part_end - part_start, "\r\n\r\n", 4);
            if (s >= 0) {
                size_t hlen = (size_t)s;
                size_t bbeg = part_start + (size_t)s + 4;
                const char* hblock = (const char*)(data + part_start);
                const char* hv = mp_header_value(hblock, hlen, "Content-Disposition", &hvl);
                if (hv) {
                    mp->name = mp_disp_param(hv, hvl, "name");
                    mp->filename = mp_disp_param(hv, hvl, "filename");
                }
                hv = mp_header_value(hblock, hlen, "Content-Type", &hvl);
                if (hv)
                    mp->content_type = mp_strndup(hv, hvl);
                {
                    size_t blen = part_end - bbeg;
                    mp->body = new_nstr(blen + 1);
                    if (mp->body) {
                        if (blen)
                            memcpy(mp->body->data, data + bbeg, blen);
                        mp->body->data[blen] = '\0';
                        mp->body->written = blen;
                    }
                }
            }
        }
        if (!mp->name)
            mp->name = mp_strndup("", 0);
        if (!mp->filename)
            mp->filename = mp_strndup("", 0);
        if (!mp->content_type)
            mp->content_type = mp_strndup("", 0);
        if (!mp->body)
            mp->body = new_nstr(1);
        list_push(parts, mp, mp_part_free);

        cursor = part_end + cdlen; /* skip "\r\n--boundary" to the next part/closing */
    }
    return parts;
}

/* Append n bytes to buf at *pos (sizing pass when buf == NULL). */
...
/* Append a NUL-terminated string (length via strlen). */
static void mp_puts(char* buf, size_t* pos, const char* s) {
    if (s)
        mp_put(buf, pos, s, strlen(s));
}

/* Emit the full multipart body into buf (or size it when buf == NULL). */
static void mp_emit(char* buf, size_t* pos, LIST* parts, const char* dash) {
    list_foreach(node, parts) {
        const N_HTTP_MULTIPART_PART* mp = (const N_HTTP_MULTIPART_PART*)node->ptr;
        if (!mp)
            continue;
        mp_puts(buf, pos, dash);
        mp_puts(buf, pos, "\r\nContent-Disposition: form-data; name=\"");
        mp_puts(buf, pos, mp->name ? mp->name : "");
        mp_puts(buf, pos, "\"");
        if (mp->filename && mp->filename[0]) {
            mp_puts(buf, pos, "; filename=\"");
            mp_puts(buf, pos, mp->filename);
            mp_puts(buf, pos, "\"");
        }
        mp_puts(buf, pos, "\r\n");
        if (mp->content_type && mp->content_type[0]) {
            mp_puts(buf, pos, "Content-Type: ");
            mp_puts(buf, pos, mp->content_type);
            mp_puts(buf, pos, "\r\n");
        }
        mp_puts(buf, pos, "\r\n");
        if (mp->body && mp->body->written)
            mp_put(buf, pos, mp->body->data, mp->body->written);
        mp_puts(buf, pos, "\r\n");
    }
    mp_puts(buf, pos, dash);
    mp_puts(buf, pos, "--\r\n");
...
N_STR* n_http_build_multipart(LIST* parts, const char* boundary) {
    ...
    char* buf;
    N_STR* out = NULL;
    if (!parts || !boundary || !boundary[0])
        return NULL;
    if (strlen(boundary) + 2 >= sizeof(dash))
        return NULL;
    snprintf(dash, sizeof(dash), "--%s", boundary);

    mp_emit(NULL, &total, parts, dash); /* sizing pass */
    Malloc(buf, char, total + 1);
    if (!buf)
        return NULL;
    total = 0;
    mp_emit(buf, &total, parts, dash); /* fill pass */
    buf[total] = '\0';
    char_to_nstr_ex(buf, (NSTRBYTE)total, &out);
    free(buf);
    return out;
}
...
N_WS_CONN* n_ws_connect(const char* host, const char* port, const char* path, int use_ssl) {
    ...
    {
        int ai = 0;
        while (ai < (int)sizeof(accept_val) - 1 && accept_hdr[ai] && accept_hdr[ai] != '\r' && accept_hdr[ai] != '\n') {
...
/*! upper bound on a single parsed WebSocket frame payload, to bound a proxy's
 *  reassembly buffer against a hostile or corrupt length field */
#define N_WS_FRAME_MAX_PAYLOAD (16U * 1024U * 1024U)

/**
 * @brief Parse one WebSocket frame from a byte buffer (stateless, no allocation).
 * @param buf The input bytes (one or more frames, possibly partial).
 * @param len Number of readable bytes in @p buf.
 * @param frame Receives the parsed header and a payload pointer aliasing @p buf.
 * @param consumed Receives the total frame size in bytes on a complete frame.
 * @return 1 on a complete frame, 0 when more bytes are needed, -1 when malformed
 *         or the payload exceeds N_WS_FRAME_MAX_PAYLOAD.
 */
int n_ws_frame_parse(const unsigned char* buf, size_t len, N_WS_FRAME* frame, size_t* consumed) {
    ...
    frame->rsv = (buf[0] >> 4) & 0x07;
    frame->opcode = buf[0] & 0x0F;
    frame->masked = (buf[1] & 0x80) ? 1 : 0;
    len7 = buf[1] & 0x7F;
    if (len7 < 126) {
        plen = (uint64_t)len7;
    } else if (len7 == 126) {
        if (len < 4)
            return 0;
        plen = ((uint64_t)buf[2] << 8) | (uint64_t)buf[3];
        hdr = 4;
    } else {
        int i;
        if (len < 10)
        ...
        plen = 0;
        for (i = 0; i < 8; i++)
            plen = (plen << 8) | (uint64_t)buf[2 + i];
        hdr = 10;
    }
    if (plen > N_WS_FRAME_MAX_PAYLOAD)
        return -1;
    if (frame->masked) {
        if (len < hdr + 4)
            return 0;
...
void n_ws_unmask(unsigned char* dst, const unsigned char* src, size_t len, const unsigned char mask[4]) {
    ...
    if (!dst || !src || !mask)
        return;
    for (i = 0; i < len; i++)
        dst[i] = (unsigned char)(src[i] ^ mask[i & 3]);
}
...
size_t n_ws_frame_build(int fin, int opcode, int do_mask, const unsigned char* payload, size_t len, const unsigned char mask_key[4], unsigned char* out, size_t out_cap) {
    size_t pos = 0;
    unsigned char b1 = (unsigned char)(do_mask ? 0x80 : 0);
    if (!out || out_cap < 2)
        return 0;
    out[pos++] = (unsigned char)((fin ? 0x80 : 0) | (opcode & 0x0F));
    if (len <= 125) {
        out[pos++] = (unsigned char)(b1 | (unsigned char)len);
    } else if (len <= 65535) {
        if (out_cap < pos + 3)
            return 0;
        out[pos++] = (unsigned char)(b1 | 126);
        out[pos++] = (unsigned char)((len >> 8) & 0xFF);
        out[pos++] = (unsigned char)(len & 0xFF);
    } else {
        int i;
        if (out_cap < pos + 9)
            return 0;
        out[pos++] = (unsigned char)(b1 | 127);
        for (i = 7; i >= 0; i--)
            out[pos++] = (unsigned char)((len >> (8 * i)) & 0xFF);
    }
    if (do_mask) {
        if (!mask_key || out_cap < pos + 4)
            return 0;
    ...
    if (do_mask && mask_key) {
        size_t i;
        for (i = 0; i < len; i++)
            out[pos + i] = (unsigned char)((payload ? payload[i] : 0) ^ mask_key[i & 3]);
...
/**
 * @brief normalize a path by removing "." and ".." segments (RFC 3986 style)
 * @param path the input path (may be NULL or empty)
 * @param out destination buffer
 * @param outsz size of out
 */
static void _n_url_normalize_path(const char* path, char* out, size_t outsz) {
    const char* seg_starts[256];
    size_t seg_lens[256];
    int nb = 0;
    const char* p = (path && path[0]) ? path : "/";
    size_t pos = 0;
    int i;
    /* split on '/', applying the dot-segment rules */
    while (*p) {
        const char* slash;
        size_t slen;
        if (*p == '/') {
        ...
        slash = strchr(p, '/');
        slen = slash ? (size_t)(slash - p) : strlen(p);
        if (slen == 1 && p[0] == '.') {
            /* skip "." */
        } else if (slen == 2 && p[0] == '.' && p[1] == '.') {
            if (nb > 0) nb--; /* pop one segment */
        } else if (nb < 256) {
            seg_starts[nb] = p;
            seg_lens[nb] = slen;
            nb++;
    ...
    /* rebuild with a leading slash */
    if (outsz == 0) return;
    out[pos++] = '/';
    for (i = 0; i < nb && pos < outsz - 1; i++) {
        size_t k;
        if (i > 0 && pos < outsz - 1) out[pos++] = '/';
        for (k = 0; k < seg_lens[i] && pos < outsz - 1; k++)
            out[pos++] = seg_starts[i][k];
    }
    /* preserve a trailing slash from the original (it can be significant) */
    if (path && path[0] && path[strlen(path) - 1] == '/' && pos < outsz - 1 && (nb > 0))
        out[pos++] = '/';
    out[pos] = '\0';
}

/**
 * @brief produce a canonical URL string for deduplication
 *
 * Lowercases the scheme and host, drops the default port (80 for http, 443 for
 * https), removes "." and ".." path segments, and keeps the query verbatim.
 *
 * @param u a parsed URL
 * @return a newly allocated N_STR canonical form, or NULL on error
 */
N_STR* n_url_canonicalize(const N_URL* u) {
    ...
    size_t i;
    if (!u) return NULL;
    snprintf(scheme, sizeof(scheme), "%s", u->scheme ? u->scheme : "http");
    snprintf(host, sizeof(host), "%s", u->host ? u->host : "");
    for (i = 0; scheme[i]; i++) scheme[i] = (char)tolower((unsigned char)scheme[i]);
    for (i = 0; host[i]; i++) host[i] = (char)tolower((unsigned char)host[i]);
    _n_url_normalize_path(u->path, path, sizeof(path));
    result = new_nstr(2560);
    if (!result) return NULL;
    nstrprintf(result, "%s://%s", scheme, host);
    if (u->port > 0) {
        int dp = (strcmp(scheme, "https") == 0) ? 443 : 80;
        if (u->port != dp) nstrprintf_cat(result, ":%d", u->port);
    }
    nstrprintf_cat(result, "%s", path);
    if (u->query && u->query[0]) nstrprintf_cat(result, "?%s", u->query);
    return result;
}

/**
 * @brief parse and canonicalize a URL string in one call
 * @param url the URL string
 * @return a newly allocated N_STR canonical form, or NULL on error
 */
N_STR* n_url_canonicalize_string(const char* url) {
    N_URL* u = n_url_parse(url);
    N_STR* out;
    if (!u) return NULL;
    out = n_url_canonicalize(u);
    n_url_free(&u);
    return out;
}

/**
 * @brief test whether a reference string begins with its own URI scheme
 * @param s the reference string
 * @return 1 when s starts with "scheme:" (alpha followed by alnum/+/-/.), 0 otherwise
 */
static int _n_url_has_scheme(const char* s) {
    if (!s || !isalpha((unsigned char)s[0])) return 0;
    size_t i = 1;
    while (s[i] && (isalnum((unsigned char)s[i]) || s[i] == '+' || s[i] == '-' || s[i] == '.')) i++;
    return s[i] == ':';
}

/**
 * @brief resolve a (possibly relative) URL reference against an absolute base
 *
 * Implements RFC 3986 section 5 reference resolution: a base URL plus a
 * reference found in a document (an href, src, or action) produce an absolute
 * URL. Handles absolute references (returned verbatim), scheme-relative
 * ("//host/path"), absolute-path ("/path"), query-only ("?q"), fragment-only
 * (resolves to the base resource), and relative-path references (merged against
 * the base path). The fragment is dropped (it is not part of the resource
 * identity for crawling) and "." / ".." path segments are removed. The result
 * is not lowercased or otherwise canonicalized; pass it through
 * n_url_canonicalize_string for a dedup key.
 *
 * @param base an absolute base URL (scheme://host/...). NULL returns NULL.
 * @param ref the reference to resolve. NULL is treated as the empty string.
 * @return a newly allocated absolute N_STR URL, or NULL on error.
 */
N_STR* n_url_resolve(const char* base, const char* ref) {
    if (!base) return NULL;

    /* fragment-stripped working copy of the reference */
    char* refbuf = strdup(ref ? ref : "");
    if (!refbuf) return NULL;
    char* frag = strchr(refbuf, '#');
    if (frag) *frag = '\0';

    /* absolute reference: it carries its own scheme, return it verbatim */
    if (_n_url_has_scheme(refbuf)) {
        N_STR* out = new_nstr(strlen(refbuf) + 1);
        if (out) nstrprintf(out, "%s", refbuf);
        free(refbuf);
        return out;
    }

    N_URL* b = n_url_parse(base);
    if (!b) {
        free(refbuf);
        return NULL;
    }

    /* split the reference into its path and query parts at the first '?' */
    char* refq = strchr(refbuf, '?');
    const char* ref_query = NULL;
    if (refq) {
        *refq = '\0';
        ref_query = refq + 1;
    }

    /* base authority = host plus a non-elided port */
    char authority[300];
    if (b->port > 0)
        snprintf(authority, sizeof(authority), "%s:%d", b->host ? b->host : "", b->port);
    else
        snprintf(authority, sizeof(authority), "%s", b->host ? b->host : "");

    char rawpath[4096];
    const char* t_scheme = b->scheme ? b->scheme : "http";
    const char* t_query = NULL;

    if (refbuf[0] == '\0') {
        /* empty path: same resource; keep the base query unless one was given */
        snprintf(rawpath, sizeof(rawpath), "%s", b->path ? b->path : "/");
        t_query = refq ? ref_query : b->query;
    } else if (refbuf[0] == '/' && refbuf[1] == '/') {
        /* scheme-relative: base scheme, new authority + path from the reference */
        const char* a = refbuf + 2;
        const char* slash = strchr(a, '/');
        if (slash) {
            snprintf(authority, sizeof(authority), "%.*s", (int)(slash - a), a);
            snprintf(rawpath, sizeof(rawpath), "%s", slash);
        } else {
            snprintf(authority, sizeof(authority), "%s", a);
            snprintf(rawpath, sizeof(rawpath), "/");
        }
        t_query = ref_query;
    } else if (refbuf[0] == '/') {
        /* absolute-path reference */
        snprintf(rawpath, sizeof(rawpath), "%s", refbuf);
        t_query = ref_query;
    } else {
        /* relative-path reference: merge against the base path */
        const char* bp = (b->path && b->path[0]) ? b->path : "/";
        const char* lastslash = strrchr(bp, '/');
        size_t prefixlen = lastslash ? (size_t)(lastslash - bp) + 1 : 0;
        snprintf(rawpath, sizeof(rawpath), "%.*s%s", (int)prefixlen, bp, refbuf);
        t_query = ref_query;
    }

    char normpath[4096];
    _n_url_normalize_path(rawpath, normpath, sizeof(normpath));

    N_STR* out = new_nstr(strlen(authority) + sizeof(normpath) + 32);
    if (out) {
        nstrprintf(out, "%s://%s%s", t_scheme, authority, normpath);
        if (t_query && t_query[0]) nstrprintf_cat(out, "?%s", t_query);
    }
    n_url_free(&b);
    free(refbuf);
    return out;
...
int n_proxy_connect_socks5(const N_PROXY_CFG* proxy,
    ...
    if (method_resp[1] == 0x02 && use_auth) {
        ...
        /* SOCKS5 username/password auth (RFC 1929) transmits the credentials
         * in cleartext by design; the protocol has no in-band encryption. A
         * caller needing confidentiality must tunnel this over an encrypted
         * link (e.g. an HTTPS CONNECT proxy). Not a fixable defect here. */
...
/**
 * @brief Adopt an already-connected socket fd into a fresh client NETWORK.
 *
 * Wraps a connected socket file descriptor (for example one returned by
 * n_proxy_connect_tunnel / n_proxy_connect_socks5) in a newly allocated client
 * NETWORK so the blocking send/recv helpers (send_data / netw_read_all / ...)
 * can run over it. The NETWORK is created with the plain (non-SSL) send/recv
 * vtable; promote it with netw_ssl_start_client + netw_ssl_do_handshake to speak
 * TLS over the adopted socket. On success the NETWORK owns @p fd and closes it
 * in netw_close; on failure @p fd is left open for the caller to close.
 *
 * @param netw output: receives the new NETWORK. Must point to NULL on entry.
 * @param fd a connected socket descriptor (must not be INVALID_SOCKET).
 * @param host hostname recorded for logging, or NULL.
 * @param port port string recorded for logging, or NULL.
 * @return TRUE on success, FALSE on error.
 */
int netw_adopt_client_fd(NETWORK** netw, SOCKET fd, const char* host, const char* port) {
    __n_assert(netw, return FALSE);
    if ((*netw)) {
        n_log(LOG_ERR, "netw_adopt_client_fd: target NETWORK must be empty");
        return FALSE;
    }
    if (fd == INVALID_SOCKET) {
        n_log(LOG_ERR, "netw_adopt_client_fd: invalid socket");
        return FALSE;
    }
    if (netw_init_wsa(1, 2, 2) == FALSE) {
        n_log(LOG_ERR, "netw_adopt_client_fd: unable to load WSA dll's");
        return FALSE;
    }
    (*netw) = netw_new(MAX_LIST_ITEMS, MAX_LIST_ITEMS);
    __n_assert(netw && (*netw), return FALSE);
    ...
    (*netw)->link.sock = fd;
    (*netw)->link.ip = strdup(host ? host : "");
    if (!(*netw)->link.ip) {
        netw_close(netw);
        return FALSE;
    }
    (*netw)->link.port = strdup(port ? port : "");
    if (!(*netw)->link.port) {
        netw_close(netw);
        return FALSE;
    }
    ...
    netw_set((*netw), NETW_CLIENT | NETW_RUN | NETW_THR_ENGINE_STOPPED);
    n_log(LOG_DEBUG, "netw_adopt_client_fd: adopted socket %d for %s:%s", (int)fd, (*netw)->link.ip, (*netw)->link.port);
    return TRUE;
