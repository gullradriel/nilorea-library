...
#define netstrerror(code) ({                                                                                    \
    wchar_t* __netstrerr_ws = NULL;                                                                             \
    FormatMessageW(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, \
                   NULL, (DWORD)(code),                                                                         \
                   MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT),                                                   \
                   (LPWSTR) & __netstrerr_ws, 0, NULL);                                                         \
    char* netstr = wchar_to_char(__netstrerr_ws);                                                               \
    LocalFree(__netstrerr_ws);                                                                                  \
    netstr;                                                                                                     \
})
...
int netw_connect_ex(NETWORK** netw, char* host, char* port, size_t send_list_limit, size_t recv_list_limit, int ip_version, char* ssl_key_file, char* ssl_cert_file) {
    ...
    for (rp = (*netw)->link.rhost; rp != NULL; rp = rp->ai_next) {
        ...
        net_status = connect(sock, rp->ai_addr, (socklen_t)rp->ai_addrlen);
        if (net_status == -1) {
            error = neterrno;
            errmsg = netstrerror(error);
...
int netw_close(NETWORK** netw) {
    ...
    if ((*netw)->deplete_queues_timeout > 0) {
        ...
        if (thr_engine_status == NETW_THR_ENGINE_STARTED) {
            ...
            countdown = (it >= max_iterations) ? 0 : 1;
            netw_get_state((*netw), &state, &thr_engine_status);
        ...
        if (status > 0) {
            ...
            switch (ssl_error) {
                case SSL_ERROR_SYSCALL:
                    ...
                    n_log(LOG_ERR, "socket %d SSL_write syscall error: connection closed by peer", s);
                    break;
                case SSL_ERROR_SSL:
