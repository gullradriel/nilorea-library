...
/*!\fn NETWORK *netw_new( size_t send_list_limit , size_t recv_list_limit )
...
/*!\fn int netw_get_state( NETWORK *netw , uint32_t *state , int *thr_engine_status )
...
/*!\fn int deplete_send_buffer( int fd , int timeout )
...
/*!\fn NETWORK *netw_accept_from_ex( NETWORK *from , size_t send_list_limit , size_t recv_list_limit , int blocking , int *retval )
...
NETWORK* netw_accept_from_ex(NETWORK* from, size_t send_list_limit, size_t recv_list_limit, int blocking, int* retval) {
    ...
    if (blocking > 0) {
        ...
        if (retval != NULL)
            (*retval) = error;
        if (tmp < 0) {
            n_log(LOG_ERR, "accept returned an invalid socket (-1)");
            netw_close(&netw);
            return NULL;
        }
        netw->link.is_blocking = 0;
...
/*!\fn ssize_t send_data( void *netw, char *buf, uint32_t n )
...
ssize_t send_data(void* netw, char* buf, uint32_t n) {
    ...
    while (bcount < n)  // loop until all sent
    {
        ...
        ssize_t retry_count = CALL_RETRY(bs, send(s, tmp_buf, (size_t)(n - bcount), NETFLAGS), NETW_MAX_RETRIES, 10);
        error = neterrno;
...
/*!\fn ssize_t recv_data( void *netw, char *buf, uint32_t n )
...
ssize_t recv_data(void* netw, char* buf, uint32_t n) {
    ...
    while (bcount < n)  // loop until all received
    {
        ...
        ssize_t retry_count = CALL_RETRY(br, recv(s, tmp_buf, (size_t)(n - bcount), NETFLAGS), NETW_MAX_RETRIES, 10);
        error = neterrno;
        if (retry_count == NETW_MAX_RETRIES) {
...
#ifdef HAVE_OPENSSL
/*!\fn ssize_t send_ssl_data( void *netw, char *buf, uint32_t n )
 *\brief send data onto the socket
...
/*!\fn ssize_t recv_ssl_data( void *netw, char *buf, uint32_t n )
 * \brief recv data from the socket
 * \param netw connected NETWORK
 * \param buf pointer to buffer
 * \param n number of characters we want
...
/*!\fn ssize_t send_php( SOCKET s, int _code , char *buf, int n )
 *\brief send data onto the socket
...
/*!\fn ssize_t recv_php( SOCKET s, int *_code , char **buf )
 *\brief recv data from the socket
 *\param s connected socket
...
ssize_t recv_php(SOCKET s, int* _code, char** buf) {
    ...
    if (tmpnb <= INT_MIN || tmpnb >= INT_MAX) {
        n_log(LOG_ERR, "Code received ( %ld ) too big or too little to be  valid code on socket %d\n", tmpnb, s);
        return FALSE;
    }
...
/*!\fn int netw_get_queue_status( NETWORK *netw, size_t *nb_to_send, size_t *nb_to_read )
 *
...
/*!\fn NETWORK_POOL *netw_new_pool( size_t nb_min_element )
...
/*!\fn int netw_get_url_from_http_request(const char *request, char *url, size_t size)
...
/*!\fn int netw_get_http_date(char *buffer, size_t buffer_size)
