...
/* Lifecycle counters. Exposed via netw_msg_stats_get(). Not
 * thread-atomic, sampled only between ticks from the main thread,
 * and advisory telemetry, not a correctness signal. */
...
void netw_msg_stats_get(long long* created, long long* deleted) {
    if (created) *created = g_netw_msg_created;
    if (deleted) *deleted = g_netw_msg_deleted;
}

/* Process-wide byte counters for the two netw_*_msg pump points
 * (accumulated in n_network.c). Exposed from the same header; same
 * advisory-only / non-atomic contract. */
extern long long g_netw_bytes_sent; /* defined in n_network.c */
extern long long g_netw_bytes_recv; /* defined in n_network.c */
void netw_bytes_stats_get(long long* sent, long long* recv) {
    if (sent) *sent = g_netw_bytes_sent;
    if (recv) *recv = g_netw_bytes_recv;
}
...
/**
 *@brief Add a binary blob to the message (clean alias for a raw N_STR
 *       payload, distinguishes binary data from character strings at
 *       call sites).
 *@param msg destination NETW_MSG
 *@param bytes pointer to the payload (copied into an internal N_STR)
 *@param len number of bytes to copy
 *@return TRUE on success, FALSE on any allocation / argument failure
 *
 * Internally this is the same wire format as add_nstrptr_to_msg, the
 * split exists so consumers that ship packed binary records (entity
 * wires, batched snapshots, path waypoints, etc.) can express their
 * intent without faking a C string. A future `NETW_MSG` framing
 * revision that removes the redundant N_STR `length` field would
 * save 4 B per blob at the receive side; call sites using
 * add_bytes_to_msg would pick that saving up for free.
 */
int add_bytes_to_msg(NETW_MSG* msg, const void* bytes, size_t len) {
    __n_assert(msg, return FALSE);
    __n_assert(bytes, return FALSE);

    /* new_nstr(N) sets length=N and allocates N+1 bytes. The wire decoder
     * requires written < length (length reserves room for a trailing NUL),
     * so we ask for len+1 to keep the invariant when written == len. */
    N_STR* blob = new_nstr(len + 1);
    __n_assert(blob, return FALSE);
    if (len > 0) memcpy(blob->data, bytes, len);
    blob->data[len] = '\0';
    blob->written = len;

    if (list_push(msg->tabstr, blob, free_nstr_ptr) == FALSE) {
        free_nstr(&blob);
        return FALSE;
    }
    return TRUE;
...
/**
 *@brief Get a binary blob from the message (counterpart of
 *       add_bytes_to_msg). The payload is handed to the caller as a
 *       plain-malloc'd buffer + length; the caller `free`s it when
 *       done.
 *@param msg source NETW_MSG
 *@param out_bytes receives the allocated payload pointer
 *@param out_len receives the payload size in bytes
 *@return TRUE on success, FALSE on missing entry / empty payload
 */
int get_bytes_from_msg(NETW_MSG* msg, void** out_bytes, size_t* out_len) {
    __n_assert(msg, return FALSE);
    __n_assert(out_bytes, return FALSE);
    __n_assert(out_len, return FALSE);

    N_STR* val = list_shift(msg->tabstr, N_STR);
    if (!val) return FALSE;

    if (!val->data || val->written == 0) {
        FreeNoLog(val->data);
        Free(val);
        *out_bytes = NULL;
        *out_len = 0;
    ...
    /* Hand the allocation to the caller, reuses the N_STR buffer to
     * avoid an extra copy. The caller frees with plain free(). */
    *out_bytes = val->data;
    *out_len = val->written;
    val->data = NULL;
    Free(val);
