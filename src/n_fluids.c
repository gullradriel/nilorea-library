...
/**
 *@brief destroy a fluid structure
...
int destroy_n_fluid(N_FLUID** fluid) {
    ...
    list_destroy(&(*fluid)->integrate_chunk_list);
    list_destroy(&(*fluid)->solveIncompressibility_chunk_list);
    list_destroy(&(*fluid)->advectVel_chunk_list);
    list_destroy(&(*fluid)->advectSmoke_chunk_list);
...
N_FLUID* new_n_fluid(double density, double gravity, size_t numIters, double dt, double overRelaxation, size_t sx, size_t sy) {
    ...
    long int nb_cores_l = get_nb_cpu_cores();
    if (nb_cores_l <= 0) {
        nb_cores_l = 1;
    }
    size_t nb_cores = (size_t)nb_cores_l;

    // precalculate and allocated N_PROC_PARAMS lists for threaded computing
    fluid->integrate_chunk_list = new_generic_list(nb_cores);
    fluid->solveIncompressibility_chunk_list = new_generic_list(nb_cores);
    fluid->advectVel_chunk_list = new_generic_list(nb_cores);
    fluid->advectSmoke_chunk_list = new_generic_list(nb_cores);

    size_t steps = (fluid->numX / nb_cores > 0) ? fluid->numX / nb_cores : 1;
    ...
    /* set the last batch to cover the full x range */
    if (fluid->integrate_chunk_list->end) {
    ...
    /* set the last batch to cover the full x range */
    if (fluid->solveIncompressibility_chunk_list->end) {
    ...
    /* set the last batch to cover the full x range */
    if (fluid->advectVel_chunk_list->end) {
    ...
    /* set the last batch to cover the full x range */
    if (fluid->advectSmoke_chunk_list->end) {
...
double n_fluid_sampleField(N_FLUID* fluid, double x, double y, uint32_t field) {
    ...
    x = MAX(MIN(x, (double)fluid->numX * fluid->h), fluid->h);
    y = MAX(MIN(y, (double)fluid->numY * fluid->h), fluid->h);
    ...
    double x0 = MIN(floor((x - dx) * h1), (double)(fluid->numX - 1));
    double tx = ((x - dx) - x0 * fluid->h) * h1;
    double x1 = MIN(x0 + 1, (double)(fluid->numX - 1));

    double y0 = MIN(floor((y - dy) * h1), (double)(fluid->numY - 1));
    double ty = ((y - dy) - y0 * fluid->h) * h1;
    double y1 = MIN(y0 + 1, (double)(fluid->numY - 1));

    double sx = 1.0 - tx;
    double sy = 1.0 - ty;

    double val = sx * sy * f[(size_t)(x0 * (double)n + y0)] +
                 tx * sy * f[(size_t)(x1 * (double)n + y0)] +
                 tx * ty * f[(size_t)(x1 * (double)n + y1)] +
                 sx * ty * f[(size_t)(x0 * (double)n + y1)];
    ...
    for (size_t i = params->x_start; i < params->x_end; i++) {
        for (size_t j = params->y_start; j < params->y_end; j++) {
            ...
            if (!_z(fluid, s[index]) && !_z(fluid, s[(i - 1) * n + j]) && j < fluid->numY - 1) {
                double x = (double)i * fluid->h;
                double y = (double)j * fluid->h + h2;
            ...
            if (!_z(fluid, s[index]) && !_z(fluid, s[index - 1]) && i < fluid->numX - 1) {
                double x = (double)i * fluid->h + h2;
                double y = (double)j * fluid->h;
    ...
    for (size_t i = params->x_start; i < params->x_end; i++) {
        for (size_t j = params->y_start; j < params->y_end; j++) {
            ...
            if (!_z(fluid, s[index])) {
                ...
                double x = (double)i * fluid->h + h2 - fluid->dt * u;
                double y = (double)j * fluid->h + h2 - fluid->dt * v;
...
int n_fluid_setObstacle(N_FLUID* fluid, double x, double y, double vx, double vy, double r) {
    ...
    for (size_t i = 1; i < fluid->numX - 2; i++) {
        for (size_t j = 1; j < fluid->numY - 2; j++) {
            double dx = ((double)i + 0.5) - x;
            double dy = ((double)j + 0.5) - y;
...
/**
 * @brief get funky colors for the fluid
 * @param fluid targeted N_FLUID
...
ALLEGRO_COLOR n_fluid_getSciColor(N_FLUID* fluid, double val, double minVal, double maxVal) {
    ...
    double m = 0.25;
    size_t num = (size_t)floor(val / m);
    double s = (val - (double)num * m) / m;
    ...
    return al_map_rgb_f((float)r, (float)g, (float)b);
}
...
int n_fluid_draw(N_FLUID* fluid) {
    ...
    for (size_t i = 0; i < fluid->numX; i++) {
        for (size_t j = 0; j < fluid->numY; j++) {
            double xd = (double)i * cScale;
            double yd = (double)j * cScale;
            double cxd = xd + cScale;
            double cyd = yd + cScale;
            ...
            if (fluid->showPaint) {
                ...
                if (fluid->showSmoke) {
                    ...
                    color = al_map_rgb_f((float)MAX(0.0, color_vec_f[0] - s), (float)MAX(0.0, color_vec_f[1] - s), (float)MAX(0.0, color_vec_f[2] - s));
                }
            } else if (fluid->showSmoke) {
                color = al_map_rgb_f((float)(1.0 - s), 0.0f, 0.0f);
            } else {
                color = al_map_rgb_f((float)s, (float)s, (float)s);
            }
            al_draw_filled_rectangle((float)xd, (float)yd, (float)cxd, (float)cyd, color);
        }
    }
    return TRUE;
}
