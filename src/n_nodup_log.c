...
int init_nodup_log(size_t max) {
    ...
    if (_n_nodup_table == NULL) {
        n_log(LOG_ERR, "Could not allocate the internal hash with size %zu", max);
        return FALSE;
    } else {
        n_log(LOG_DEBUG, "LOGGING: nodup system activated with a hash table of %zu cells", max);
    }
...
void _n_nodup_log(int LEVEL, const char* file, const char* func, int line, const char* format, ...) {
    ...
    switch (is_dup) {
        ...
        case 2:
            ...
            _n_log(LEVEL, file, func, line, "%s", node ? node->data.string : "");
            break;
    ...
    if (_n_nodup_table) {
        for (unsigned long int it = 0; it < _n_nodup_table->size; it++) {
            list_foreach(list_node, _n_nodup_table->hash_table[it]) {
                const HASH_NODE* hash_node = (const HASH_NODE*)list_node->ptr;
