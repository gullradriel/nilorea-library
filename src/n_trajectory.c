...
int trajectory_add_point(TRAJECTORY* traj, const PHYSICS* state, double time_val) {
    ...
    if (traj->nb_points >= traj->nb_points_allocated) {
        ...
        __n_assert(new_size > 0, return FALSE);
...
double trajectory_distance(TRAJECTORY* traj, double time_a, double time_b, int steps) {
    ...
    VECTOR3D prev, curr;
    /* nb_components is constrained to TRAJECTORY_2D (2) or TRAJECTORY_3D (3)
     * at construction time. Clamp to the VECTOR3D extent so the static
     * analyzer can see the bound and the per-component loop below stays
     * safely inside the [0,3) array range. */
    int nc = traj->nb_components;
    if (nc < 2) nc = 2;
    if (nc > 3) nc = 3;
    ...
    for (int step = 1; step <= steps; step++) {
        ...
        for (int i = 0; i < nc; i++) prev[i] = curr[i];
    }
