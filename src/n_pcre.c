...
N_STR* npcre_replace(N_PCRE* pcre, const char* subject, size_t subject_len, const char* replacement) {
    ...
    PCRE2_UCHAR8* outbuf = NULL;
    PCRE2_SIZE slen, rlen, cap;
    N_STR* result = NULL;

    __n_assert(pcre, return NULL);
    __n_assert(pcre->regexp, return NULL);
    __n_assert(subject, return NULL);
    __n_assert(replacement, return NULL);

    md = pcre2_match_data_create_from_pattern(pcre->regexp, NULL);
    if (!md) {
        n_log(LOG_ERR, "npcre_replace: pcre2_match_data_create_from_pattern failed");
        return NULL;
    }

    slen = (PCRE2_SIZE)subject_len;
    rlen = (PCRE2_SIZE)strlen(replacement);
    cap = slen + 64; /* initial output capacity guess (code units, incl. room for NUL) */

    for (;;) {
        PCRE2_SIZE used = cap; /* in: buffer capacity; out: length used (or required) */
        int rc;
        Malloc(outbuf, PCRE2_UCHAR8, cap);
        if (!outbuf) {
        ...
        rc = pcre2_substitute(
            pcre->regexp,
            (PCRE2_SPTR)subject, slen,
            0,
            PCRE2_SUBSTITUTE_GLOBAL | PCRE2_SUBSTITUTE_OVERFLOW_LENGTH,
            md, NULL,
            (PCRE2_SPTR)replacement, rlen,
            outbuf, &used);
        if (rc == PCRE2_ERROR_NOMEMORY) {
            /* OVERFLOW_LENGTH set used to the required buffer size (incl. NUL) */
            FreeNoLog(outbuf);
            outbuf = NULL;
            cap = used;
            continue;
        ...
        if (rc < 0) {
            npcre_print_error(LOG_ERR, rc);
            FreeNoLog(outbuf);
            pcre2_match_data_free(md);
            return NULL;
        }
        /* success: used is the result length, excluding the trailing NUL */
        char_to_nstr_ex((char*)outbuf, (NSTRBYTE)used, &result);
        FreeNoLog(outbuf);
        break;
    }
