...
int npcre_match( char *str, N_PCRE *pcre )
{
	...
	if ( rc < 0 )
	{
		switch( rc )
        {
            case PCRE_ERROR_NOMATCH      : /* n_log( LOG_DEBUG , "String did not match the pattern");*/   break;
            case PCRE_ERROR_NULL         : n_log( LOG_DEBUG , "Something was null");                      break;
            case PCRE_ERROR_BADOPTION    : n_log( LOG_DEBUG , "A bad option was passed");                 break;
            case PCRE_ERROR_BADMAGIC     : n_log( LOG_DEBUG , "Magic number bad (compiled re corrupt?)"); break;
            case PCRE_ERROR_UNKNOWN_NODE : n_log( LOG_DEBUG , "Something kooky in the compiled regexp");  break;
            case PCRE_ERROR_NOMEMORY     : n_log( LOG_DEBUG , "Ran out of memory");                       break;
            default                      : n_log( LOG_DEBUG , "Unknown error");                           break;
