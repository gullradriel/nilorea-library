...
int get_computer_name(char* computer_name, size_t len) {
    if (!computer_name || len == 0) return FALSE;
    ...
    if (GetComputerName(infoBuf, &bufCharCount)) {
        ...
        strncpy(computer_name, "Unknown_Host_Name", len - 1);
        computer_name[len - 1] = '\0';
        return FALSE;
    }
#else
...
#ifdef __linux__
    snprintf(procbuf, PATH_MAX, "/proc/%d/exe", (int)getpid());
#elif defined __sun
    snprintf(procbuf, PATH_MAX, "/proc/%d/path/a.out", (int)getpid());
#endif
...
/**
 * @brief launch a command and return output and status
 * @param cmd The command to launch
 * @param read_buf_size popen read buf. Also serves as a minimum size for the dynamically allocated returned output if not already allocated
...
int n_popen(char* cmd, size_t read_buf_size, void** nstr_output, int* ret) {
    ...
    if (read_buf_size > 65536) {
        n_log(LOG_ERR, "read_buf_size %zu exceeds safe stack limit", read_buf_size);
        return FALSE;
    }
    ...
    while (fgets(read_buf, (int)read_buf_size, fp) != NULL) {
        size_t length = strlen(read_buf);
        if (length > 0 && read_buf[length - 1] == '\n') {
            read_buf[length - 1] = '\0';
        }
...
#if defined(_WIN32) || defined(__MINGW32__) || defined(__windows__)
    LPCH env_block = GetEnvironmentStrings();
    if (!env_block) return;
    for (LPCH var = env_block; *var != '\0'; var += strlen(var) + 1) {
        n_log(loglevel, "env: %s", var);
    }
 ...
 *@param buf the variable which will receive the string. Must be large enough to hold all arguments.
 *@warning No bounds checking is performed. The caller must ensure buf is large enough.
 */
