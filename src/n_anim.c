...
int destroy_anim_lib(ANIM_LIB** lib) {
    ...
    for (uint32_t it = 0; it < (*lib)->nb_max_gfxs; it++) {
        if ((*lib)->gfxs[it]) {
            FreeNoLog((*lib)->gfxs[it]->frames);
            if ((*lib)->gfxs[it]->bmp) al_destroy_bitmap((*lib)->gfxs[it]->bmp);
        }
...
int delete_bmp_from_lib(ANIM_LIB* lib, unsigned int id) {
    ...
    if (id >= lib->nb_max_gfxs) return FALSE;
    if (lib->gfxs[id]) {
        FreeNoLog(lib->gfxs[id]->frames);
        if (lib->gfxs[id]->bmp) al_destroy_bitmap(lib->gfxs[id]->bmp);
        FreeNoLog(lib->gfxs[id]);
    }
    return TRUE;
...
int add_bmp_to_lib(ANIM_LIB* lib, unsigned int pos, char* file, char* resfile) {
    ...
    check_fscanf = fscanf(data, "%d", &check_id);
    if (check_fscanf == 1 && check_id == 211282) {
        Malloc(gfx, ANIM_GFX, 1);
        __n_assert(gfx, fclose(data); return FALSE);
    } else {
    ...
    if (fscanf(data, "%u %u %u", &gfx->w, &gfx->h, &gfx->nb_frames) != 3) {
    ...
    if (gfx->nb_frames >= UINT_MAX) {
        n_log(LOG_ERR, "file %s: invalid number of frames: %u", resfile, gfx->nb_frames);
        Free(gfx);
        fclose(data);
        return FALSE;
    }
    // file stores last_frame_index (0-based), convert to count
    gfx->nb_frames += 1;

    Malloc(gfx->frames, ANIM_FRAME, gfx->nb_frames);
    if (!gfx->frames) {
        n_log(LOG_ERR, "file %s: could not allocate %d frames", resfile, gfx->nb_frames);
        Free(gfx);
        fclose(data);
        return FALSE;
    }
    ...
    for (unsigned int it = 0; it < gfx->nb_frames; it++) {
        if (fscanf(data, "%d %d %u", &gfx->frames[it].x, &gfx->frames[it].y, &gfx->frames[it].duration) != 3) {
...
int update_anim(ANIM_DATA* data, unsigned int delta_t) {
    ...
    if (data->lib->gfxs[data->id]->nb_frames <= 1)
        return TRUE;
...
int draw_anim(ANIM_DATA* data, int x, int y) {
    ...
    al_draw_bitmap_region(bmp, (float)framex, 0, (float)tilew, (float)tileh, (float)(x - px), (float)(y - py), 0);
