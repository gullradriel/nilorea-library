...
#else
#define _iso_fopen_write(path, mode) fopen((path), (mode))
#endif

/* Per-pass timing accumulators. Host resets these at frame start,
 * iso_map_draw adds elapsed microseconds per pass per invocation. See
 * the doxygen block in n_iso_engine.h for usage. */
double n_iso_pass1_us = 0.0;
double n_iso_pass2_us = 0.0;
double n_iso_pass3_us = 0.0;
double n_iso_setup_us = 0.0;

#ifdef HAVE_ALLEGRO
/* Optional primitive-color filter (sRGB-aware consumers register a
 * design->linear converter here). See header docstring. NULL = identity.
 * Gated on HAVE_ALLEGRO because the prototype mentions ALLEGRO_COLOR
 * which doesn't exist in the no-allegro test build path. */
static IsoPrimitiveColorFilter g_iso_primitive_color_filter = NULL;

void iso_set_primitive_color_filter(IsoPrimitiveColorFilter fn) {
    g_iso_primitive_color_filter = fn;
}

ALLEGRO_COLOR iso_apply_primitive_color_filter(ALLEGRO_COLOR c) {
    if (g_iso_primitive_color_filter) return g_iso_primitive_color_filter(c);
    return c;
}

/* Line-jitter compensation callback. See header doc. */
static IsoLineJitterCompensationFn g_iso_line_jitter_fn = NULL;

void iso_set_line_jitter_compensation(IsoLineJitterCompensationFn fn) {
    g_iso_line_jitter_fn = fn;
}

void iso_apply_line_jitter_compensation(float* out_dx, float* out_dy) {
    if (g_iso_line_jitter_fn) {
        g_iso_line_jitter_fn(out_dx, out_dy);
    } else {
...
/* Height-aware ISO_MAP API (Articles 747/748/934/1269/2026)
 * Core API does not require Allegro and compiles everywhere.
 */
...
ISO_MAP* iso_map_new(int width, int height, int num_terrains, int max_height) {
    ...
    /* Draw_order cache. dirty=1 forces a first build. */
    map->cached_draw_order = NULL;
    map->cached_draw_order_count = 0;
    map->cached_draw_order_cap = 0;
    map->draw_order_dirty = 1;
...
void iso_map_free(ISO_MAP** map_ptr) {
    ...
    Free(map->overlay);
    /* Cached draw_order */
    if (map->cached_draw_order) {
        free(map->cached_draw_order);
        map->cached_draw_order = NULL;
    }
    Free(*map_ptr);
} /* iso_map_free() */
 ...
 * cam_px and cam_py may be integer-snapped (`floor(cam_x * zoom)`) for
 * stable per-pixel rendering with NEAREST sampling, or sub-pixel
 * (`cam_x * zoom`) for smooth camera motion paired with MIN_LINEAR
 * bilinear sampling. The renderer uses the sub-pixel path; see
 * docs/rendering.md for details.
 *
 *@param map    the map (uses map->proj)
...
int iso_map_save(const ISO_MAP* map, const char* filename) {
    ...
    FILE* f = _iso_fopen_write(filename, "wb");
    if (!f) {
    ...
    /* header */
    const int32_t hdr[4] = {map->width, map->height, map->num_terrains, map->max_height};
...
int iso_map_set_segments(ISO_MAP* map, int mx, int my, const ISO_TILE_SEGMENT* segs, int count) {
    ...
    /* Draw_order depends on segments + heights; invalidate. */
    map->draw_order_dirty = 1;
    return 1;
}
...
/**
 *@brief Edge-aware terrain lookup for chunked rendering.
 * When (mx,my) falls just outside the chunk on a cardinal edge and the
 * caller has wired up the neighbor_terrains_* arrays, consult them so
 * transition masks can cross chunk seams. Out-of-range without a
 * neighbor array falls back to the standard 0-out-of-bounds semantics.
 * Diagonal neighbors (corners of the 8-neighborhood that fall outside
 * the chunk's own bounds) are not covered, a fully correct diagonal
 * fix would need 4 extra corner-cell fields.
 *@param map the map
 *@param mx tile X (may be just outside [0, width))
 *@param my tile Y (may be just outside [0, height))
 *@return terrain index at (mx,my), or 0 if both in-chunk and
 *        neighbor lookups fail
 */
static int _neighbor_terrain_at(const ISO_MAP* map, int mx, int my) {
    if (my >= 0 && my < map->height) {
        if (mx == -1 && map->neighbor_terrains_west)
            return map->neighbor_terrains_west[my];
        if (mx == map->width && map->neighbor_terrains_east)
            return map->neighbor_terrains_east[my];
    }
    if (mx >= 0 && mx < map->width) {
        if (my == -1 && map->neighbor_terrains_north)
            return map->neighbor_terrains_north[mx];
        if (my == map->height && map->neighbor_terrains_south)
            return map->neighbor_terrains_south[mx];
    }
    return iso_map_get_terrain(map, mx, my);
}

static int _neighbor_height_at(const ISO_MAP* map, int mx, int my) {
    if (my >= 0 && my < map->height) {
        if (mx == -1 && map->neighbor_heights_west)
            return map->neighbor_heights_west[my];
        if (mx == map->width && map->neighbor_heights_east)
            return map->neighbor_heights_east[my];
    }
    if (mx >= 0 && mx < map->width) {
        if (my == -1 && map->neighbor_heights_north)
            return map->neighbor_heights_north[mx];
        if (my == map->height && map->neighbor_heights_south)
            return map->neighbor_heights_south[mx];
    }
    return iso_map_get_height(map, mx, my);
}

static int _neighbor_should_transition(const ISO_MAP* map, int mx1, int my1, int mx2, int my2) {
    int h1 = _neighbor_height_at(map, mx1, my1);
    int h2 = _neighbor_height_at(map, mx2, my2);
    if (map->smooth_height)
        return abs(h1 - h2) <= map->smooth_slope_max;
    return h1 == h2;
}
...
void iso_map_calc_transitions_full(const ISO_MAP* map, int mx, int my, int* edge_bits, int* corner_bits) {
    ...
    /* Cardinal neighbors: only if height-connected. Uses the
     * _neighbor_* helpers so chunk-edge lookups consult the
     * neighbor_terrains_* / neighbor_heights_* arrays instead of
     * falling to the default 0-out-of-bounds and dropping the
     * cross-chunk transition. */
    int t_w = _neighbor_should_transition(map, mx, my, mx - 1, my)
                  ? _neighbor_terrain_at(map, mx - 1, my)
                  : base;
    int t_n = _neighbor_should_transition(map, mx, my, mx, my - 1)
                  ? _neighbor_terrain_at(map, mx, my - 1)
                  : base;
    int t_e = _neighbor_should_transition(map, mx, my, mx + 1, my)
                  ? _neighbor_terrain_at(map, mx + 1, my)
                  : base;
    int t_s = _neighbor_should_transition(map, mx, my, mx, my + 1)
                  ? _neighbor_terrain_at(map, mx, my + 1)
                  : base;

    /* Diagonal neighbors, see _neighbor_terrain_at comment. Diagonals
     * that fall off the chunk still return 0 (water); the visible
     * fallout is single-tile corner artifacts at chunk boundaries
     * rather than whole-edge seams. */
    int t_nw = _neighbor_should_transition(map, mx, my, mx - 1, my - 1)
                   ? _neighbor_terrain_at(map, mx - 1, my - 1)
                   : base;
    int t_ne = _neighbor_should_transition(map, mx, my, mx + 1, my - 1)
                   ? _neighbor_terrain_at(map, mx + 1, my - 1)
                   : base;
    int t_se = _neighbor_should_transition(map, mx, my, mx + 1, my + 1)
                   ? _neighbor_terrain_at(map, mx + 1, my + 1)
                   : base;
    int t_sw = _neighbor_should_transition(map, mx, my, mx - 1, my + 1)
                   ? _neighbor_terrain_at(map, mx - 1, my + 1)
                   : base;
...
static inline ALLEGRO_COLOR _compute_tile_tint(int seg_top, int max_height, float intensity, float ambient_r, float ambient_g, float ambient_b, float dyn_r, float dyn_g, float dyn_b) {
    ...
    /* The tint multiplier is sRGB-designed (ambient curve + dyn-light
     * literals tuned on a non-sRGB display). Route through the primitive
     * color filter so an sRGB-aware consumer can linearise it before it
     * multiplies the linear-decoded texel inside an sRGB FBO. Default
     * filter = identity, so the non-sRGB path is byte-identical. */
    return iso_apply_primitive_color_filter(al_map_rgba_f(r, g, b, 1.0f));
}
...
static void _draw_tile_warped(ALLEGRO_BITMAP* bmp,
                              ...
                              int tile_h,
                              ALLEGRO_COLOR tint) {
    /* al_draw_prim samples u/v against the PARENT texture and does NOT
     * auto-translate for a sub-bitmap (unlike al_draw_*_bitmap). When tiles
     * are packed into a terrain atlas (each tile an al_create_sub_bitmap),
     * passing sub-local UVs [0..tile_w] samples the atlas ORIGIN, which is
     * transparent padding, so warped (sloped) tiles render BLACK. Resolve
     * the parent + add the sub-bitmap's atlas offset to every UV and draw
     * against the parent so the warp samples the tile's real atlas region.
     * For a standalone (non-sub) bitmap the offsets are 0 and parent == bmp,
     * so this is byte-identical to the pre-atlas behaviour. */
    ALLEGRO_BITMAP* tex = al_get_parent_bitmap(bmp);
    float ox, oy;
    if (tex) {
        ox = (float)al_get_bitmap_x(bmp);
        oy = (float)al_get_bitmap_y(bmp);
    } else {
        tex = bmp;
        ox = 0.0f;
        oy = 0.0f;
    }
    float un = ox + (float)tile_w / 2.0f, vn = oy + 0.0f;
    float ue = ox + (float)tile_w, ve = oy + (float)tile_h / 2.0f;
    float us = ox + (float)tile_w / 2.0f, vs = oy + (float)tile_h;
    float uw = ox + 0.0f, vw = oy + (float)tile_h / 2.0f;
    ...
    al_draw_prim(vtx, NULL, tex, 0, 6, ALLEGRO_PRIM_TRIANGLE_LIST);
}
...
static void _draw_segment_sides(const ISO_MAP* map,
    ...
    ALLEGRO_COLOR east_col = iso_apply_primitive_color_filter(
        al_map_rgba_f(cr * east_shade * height_brightness,
                      cg * east_shade * height_brightness,
                      cb * east_shade * height_brightness, 1.0f));
    ALLEGRO_COLOR south_col = iso_apply_primitive_color_filter(
        al_map_rgba_f(cr * south_shade * height_brightness,
                      cg * south_shade * height_brightness,
                      cb * south_shade * height_brightness, 1.0f));
    /* Keep PASS 1 on ONE blender throughout to avoid held-batch
     * cross-tile contamination. Cliff wall fills use alpha=1.0 ->
     * premul == straight, and edge lines are RGB=0 -> premul ==
     * straight, so this switch is a visual no-op. */
    ...
    if (seg_bottom == 0) {
        ...
        // cppcheck-suppress redundantAssignment ; fallback chain: min(h_e,h_se) -> h_e -> h_se
        if (east_face_h < 1) east_face_h = seg_top - h_e;
        if (east_face_h < 1) east_face_h = seg_top - h_se;

        south_face_h = seg_top - (h_s < h_se ? h_s : h_se);
        // cppcheck-suppress redundantAssignment ; fallback chain: min(h_s,h_se) -> h_s -> h_se
    ...
    /* Query the client-registered jitter source. Returns (0, 0) when
     * no jitter source is active. When non-zero, we subtract it from
     * every line endpoint so the line lands at the same screen pixel
     * each frame regardless of where in the TAA jitter cycle we are.
     * Filled polygon faces deliberately KEEP the jitter: they are
     * the larger draws TAA's history blend smooths perceptually. */
    float jx = 0.0f, jy = 0.0f;
    iso_apply_line_jitter_compensation(&jx, &jy);
    ...
    if (east_face_h > 0) {
        ...
        al_draw_line(east_x - jx, east_y + drop - jy,
                     south_x - jx, south_y + drop - jy,
                     edge, 1.0f);
    }
    ...
    if (south_face_h > 0) {
        ...
        ALLEGRO_COLOR edge = al_map_rgba(0, 0, 0, 80);
        al_draw_line(west_x - jx, west_y + drop - jy,
                     south_x - jx, south_y + drop - jy,
                     edge, 1.0f);
    }
}
 ...
 *@param sprite_half_w half-width of the sprite footprint (occlusion bbox)
 *@param cam_px camera X offset in pixels
 *@param cam_py camera Y offset in pixels
 *@param zoom current zoom
 *@param out_clip_y optional out-pointer: when non-NULL and a partial
 *       overlap is detected, receives the screen-Y above which the
 *       sprite should be clipped (for soft-fade rendering). May be NULL.
...
static int _iso_object_check_occlusion(const ISO_MAP* map,
                                       ...
                                       float zoom,
                                       float* out_clip_y) {
    /* Hoist projection constants out of the inner loop. */
    const float proj_hw = map->proj.half_w;
    const float proj_hh = map->proj.half_h;
    const float proj_lift = map->proj.tile_lift;

    float phw = proj_hw * zoom;
    float phh = proj_hh * zoom;
    float lift = proj_lift * zoom;

    /* Entity head screen Y: sprite_h world units above feet */
    float head_sy = obj_sy - sprite_h * lift;
    /* Entity horizontal extent (screen pixels) */
    float ent_left = obj_sx - sprite_half_w * zoom;
    float ent_right = obj_sx + sprite_half_w * zoom;

    int base_mx = (int)floorf(obj_fx);
    int base_my = (int)floorf(obj_fy);
    int check_range = map->max_height + 2;
    int found = 0;
    float best_clip_y = 1e9f;

    for (int dy = -1; dy <= check_range; dy++) {
        for (int dx = -1; dx <= check_range; dx++) {
            /* Only check tiles drawn after the object in painter's order.
             * Filter by isometric depth (dx+dy) not just grid position. */
            int tdepth = dx + dy;
            if (tdepth < 0) continue;
            if (tdepth == 0 && dy < 0) continue;
            if (tdepth == 0 && dy == 0 && dx <= 0) continue;
            ...
            /* tile must reach above the entity's feet to potentially occlude */
            if ((float)th <= obj_fz) continue;

            /* inlined iso_map_to_screen(map, tmx, tmy, th, &tsx, &tsy) */
            float tsx = ((float)tmx - (float)tmy) * proj_hw + proj_hw;
            float tsy = ((float)tmx + (float)tmy) * proj_hh - (float)th * proj_lift;
            float tile_dx = tsx * zoom + cam_px;
            float tile_dy = tsy * zoom + cam_py;

            /* tile screen bounding box */
            ...
            /* Check horizontal overlap (entity range vs tile range) */
            if (ent_right < tile_left || ent_left > tile_right) continue;

            /* Check vertical overlap (any part of entity behind tile) */
            if (tile_top <= obj_sy && tile_bot >= head_sy) {
                found = 1;
                /* Track north vertex of occluding tile for clip-Y */
                float nx, ny;
                _iso_corner_to_screen(proj_hw, proj_hh, proj_lift,
                                      tmx, tmy, (float)th,
                                      cam_px, cam_py, zoom, &nx, &ny);
                if (ny < best_clip_y) best_clip_y = ny;
            }
        }
    }
    if (out_clip_y) *out_clip_y = best_clip_y;
    return found;
}
...
static void _draw_height_borders(const ISO_MAP* map,
    ...
    /* Subtract the client-registered jitter (e.g. the consumer's TAA
     * sub-pixel offset) from every line endpoint so these decorative
     * borders stay pixel-stable across frames while sprites and
     * filled faces continue to inherit the jitter. Returns (0, 0)
     * when no jitter source is active. */
    float jx = 0.0f, jy = 0.0f;
    iso_apply_line_jitter_compensation(&jx, &jy);
    ...
    /* Top face edges */
    if (edges.draw_se)
        al_draw_line(vx[1] - jx, vy[1] - jy,
                     vx[2] - jx, vy[2] - jy, border_col, thickness);
    if (edges.draw_sw)
        al_draw_line(vx[2] - jx, vy[2] - jy,
                     vx[3] - jx, vy[3] - jy, border_col, thickness);
    if (edges.draw_nw)
        al_draw_line(vx[0] - jx, vy[0] - jy,
                     vx[3] - jx, vy[3] - jy, border_col, thickness);
    if (edges.draw_ne)
        al_draw_line(vx[0] - jx, vy[0] - jy,
                     vx[1] - jx, vy[1] - jy, border_col, thickness);
     ...
     */
    {
        ...
        if (drop_e > 0)
            al_draw_line(vx[1] - jx, vy[1] - jy,
                         vx[1] - jx, vy[1] + drop_e - jy,
                         border_col, thickness);
        if (drop_s > 0)
            al_draw_line(vx[2] - jx, vy[2] - jy,
                         vx[2] - jx, vy[2] + drop_s - jy,
                         border_col, thickness);
        if (drop_w > 0)
            al_draw_line(vx[3] - jx, vy[3] - jy,
                         vx[3] - jx, vy[3] + drop_w - jy,
                         border_col, thickness);
    }
    ...
    if (has_underside_face && edges.draw_underside) {
        ...
        /* SE edge */
        al_draw_line(vx[1] - jx, vy[1] + dy - jy,
                     vx[2] - jx, vy[2] + dy - jy,
                     border_col, thickness);
        /* SW edge */
        al_draw_line(vx[2] - jx, vy[2] + dy - jy,
                     vx[3] - jx, vy[3] + dy - jy,
                     border_col, thickness);
    }
}
 ...
 * Step 3.5: Height borders (ground segments only)
...
void iso_map_draw(const ISO_MAP* map,
     ...
     * The caller may pass either integer-snapped (`floorf(cam_x * zoom)`) or
     * sub-pixel (`cam_x * zoom`) values. Sub-pixel is preferred when paired
     * with MIN_LINEAR bilinear sampling for smooth camera motion. Integer
     * snap pairs with NEAREST sampling for crisp pixel-art but produces
     * "world jumps by 1 pixel" stutter during motion. Chunk pixel offsets
     * are added on top of whatever value the caller passes: they remain
     * integer at zoom = 1 regardless. */
    ...
    double _iso_t_setup = al_get_time();

    /* Build segment draw order.
     *
     * The draw_order is cached on the ISO_MAP and only rebuilt when
     * the segment/height layout actually changes (`draw_order_dirty`
     * is set by iso_map_set_segments). For a static-terrain scene
     * this drops to a zero-work hit per frame. obj_order / obj_sx_arr
     * / obj_sy_arr stay file-static because objects' world positions
     * change every frame and the buffer just needs to be big enough;
     * per-map ownership isn't required. */
    int max_draw_entries = map->width * map->height * ISO_MAX_SEGMENTS_PER_TILE;
    ISO_MAP* mut_map = (ISO_MAP*)map; /* the cache fields are deliberately writable */
    if (mut_map->cached_draw_order_cap < max_draw_entries) {
        ISO_DRAW_ENTRY* grown = (ISO_DRAW_ENTRY*)realloc(
            mut_map->cached_draw_order,
            (size_t)max_draw_entries * sizeof(ISO_DRAW_ENTRY));
        if (!grown) return;
        mut_map->cached_draw_order = grown;
        mut_map->cached_draw_order_cap = max_draw_entries;
        mut_map->draw_order_dirty = 1;
    }
    if (mut_map->draw_order_dirty) {
        mut_map->cached_draw_order_count = iso_map_build_draw_order(
            map, mut_map->cached_draw_order, mut_map->cached_draw_order_cap);
        mut_map->draw_order_dirty = 0;
    }
    ISO_DRAW_ENTRY* draw_order = mut_map->cached_draw_order;
    int num_entries = mut_map->cached_draw_order_count;
    ...
    static int* obj_order = NULL;
    static float* obj_sx_arr = NULL;
    static float* obj_sy_arr = NULL;
    static int obj_buf_cap = 0;
    if (nobj > obj_buf_cap) {
        int new_cap = obj_buf_cap > 0 ? obj_buf_cap : 64;
        while (new_cap < nobj) new_cap *= 2;
        int* g1 = (int*)realloc(obj_order, (size_t)new_cap * sizeof(int));
        float* g2 = (float*)realloc(obj_sx_arr, (size_t)new_cap * sizeof(float));
        float* g3 = (float*)realloc(obj_sy_arr, (size_t)new_cap * sizeof(float));
        if (!g1 || !g2 || !g3) {
            n_log(LOG_ERR, "iso_map_draw: object buffer realloc failed");
            /* Keep whatever grew successfully; on next frame the
             * remaining one(s) will be retried. */
            if (g1) obj_order = g1;
            if (g2) obj_sx_arr = g2;
            if (g3) obj_sy_arr = g3;
            nobj = 0;
    ...
    n_iso_setup_us += (al_get_time() - _iso_t_setup) * 1.0e6;
    double _iso_t_pass1 = al_get_time();
     ...
     *
     * Wrap the entire pass in al_hold_bitmap_drawing. The previous
     * frame-wide wrap regressed because Pass 2 interleaves tile draws
     * with object-callback sprite draws (each from a different source
     * bitmap), forcing flushes. Pass 1 has no object callbacks, it's
     * pure tile draws from whatever source the caller provided. When
     * the caller groups tiles + transitions + overlays into one atlas,
     * consecutive draws share a source texture and accumulate cleanly
     * between state-change flushes. The internal al_set_blender calls
     * still flush, but runs of same-blender draws within a blender
     * regime now batch.
     *
     * `blender_is_one` tracks whether the current blender is
     * ADD/ONE/INV_ALPHA. Every set to ONE flips it true; every
     * set to ALPHA (walls, overlay, borders, hover, grid) flips it
     * false. This lets us skip the per-tile "reset to ONE" at the
     * head of each iteration when no ALPHA block ran in the
     * previous iteration, the common case for uninstrumented
     * ground tiles. */
    int blender_is_one = 1; /* set by line 2582 above the pass */
    /* al_hold_bitmap_drawing wrap around PASS 1, opt-in via
     * NILOREA_PASS1_HOLD=1. Wrapping the pass batches consecutive
     * bitmap draws (~2 ms speedup) but historically introduced
     * intermittent darker tile-mask diamond edges at the boundaries.
     * After the bitmap/primitive interleave carve-out (see below)
     * the wrap is correct end-to-end, but it remains opt-in by
     * environment variable for A/B perf comparison. */
    static int s_hold_pass1 = -1;
    static int s_hold_per_tile = -1;
    static int s_no_transitions = -1;
    static int s_no_overlay = -1;
    static int s_no_height_borders = -1;
    static int s_no_hover_grid = -1;
    static int s_tile_limit = -1;
    static int s_force_alpha = -1;
    if (s_hold_pass1 < 0) {
        /* Default is ON; set NILOREA_PASS1_HOLD=0 to opt OUT. */
        const char* e = getenv("NILOREA_PASS1_HOLD");
        s_hold_pass1 = (e && e[0] == '0') ? 0 : 1;
    }
    /* PASS 1 instrumentation knobs. Each is env-gated, default-off so
     * production builds are unchanged. Useful when investigating
     * rendering artefacts.
     *
     *   NILOREA_PASS1_HOLD_PER_TILE=1
     *       Replace the pass-wide al_hold_bitmap_drawing(true/false)
     *       wrap with a per-tile cycle. Each tile becomes its own
     *       isolated batch.
     *
     *   NILOREA_PASS1_NO_TRANSITIONS=1
     *       Skip the terrain-transitions loop entirely.
     *
     *   NILOREA_PASS1_NO_OVERLAY=1
     *       Skip the overlay-bitmap draw (ALPHA blender).
     *
     *   NILOREA_PASS1_NO_HEIGHT_BORDERS=1
     *       Skip the height-borders ALPHA draw.
     *
     *   NILOREA_PASS1_NO_HOVER_GRID=1
     *       Skip the hover-highlight and grid-overlay ALPHA draws.
     *
     *   NILOREA_PASS1_TILE_LIMIT=N
     *       Render at most N tiles in PASS 1, then break.
     *
     *   NILOREA_PASS1_FORCE_ALPHA=1
     *       Force ALPHA blender (instead of ONE) for the base-tile
     *       and transitions draws. */
    if (s_hold_per_tile < 0) {
        const char* e = getenv("NILOREA_PASS1_HOLD_PER_TILE");
        s_hold_per_tile = (e && e[0] == '1') ? 1 : 0;
    }
    if (s_no_transitions < 0) {
        const char* e = getenv("NILOREA_PASS1_NO_TRANSITIONS");
        s_no_transitions = (e && e[0] == '1') ? 1 : 0;
    }
    if (s_no_overlay < 0) {
        const char* e = getenv("NILOREA_PASS1_NO_OVERLAY");
        s_no_overlay = (e && e[0] == '1') ? 1 : 0;
    }
    if (s_no_height_borders < 0) {
        const char* e = getenv("NILOREA_PASS1_NO_HEIGHT_BORDERS");
        s_no_height_borders = (e && e[0] == '1') ? 1 : 0;
    }
    if (s_no_hover_grid < 0) {
        const char* e = getenv("NILOREA_PASS1_NO_HOVER_GRID");
        s_no_hover_grid = (e && e[0] == '1') ? 1 : 0;
    }
    if (s_tile_limit < 0) {
        const char* e = getenv("NILOREA_PASS1_TILE_LIMIT");
        s_tile_limit = (e && e[0] >= '0' && e[0] <= '9') ? atoi(e) : 0;
    }
    if (s_force_alpha < 0) {
        const char* e = getenv("NILOREA_PASS1_FORCE_ALPHA");
        s_force_alpha = (e && e[0] == '1') ? 1 : 0;
    }
    /* If per-tile hold is requested, force the pass-wide hold OFF:
     * the per-tile cycle below opens/closes its own holds. */
    int effective_hold_pass = s_hold_pass1 && !s_hold_per_tile;
    if (effective_hold_pass) {
        al_hold_bitmap_drawing(true);
        /* Batch warmup. The first batched draw after
         * al_hold_bitmap_drawing(true) can hit a slightly different
         * fragment-shading path than subsequent draws within the same
         * held context, producing darker fractional-alpha tile-mask
         * edges on the early painter-order tiles.
         *
         * Defence: prepend a fully-transparent 1-pixel draw at an
         * offscreen position so the suspect "first draw in hold
         * cycle" is absorbed into an invisible pixel. The actual
         * tile draws then all run from the second-draw-onwards
         * state, which is the consistent one. Zero visual cost
         * (transparent + offscreen) and negligible perf cost. */
        if (tile_bitmaps[0]) {
            al_draw_tinted_scaled_bitmap(tile_bitmaps[0],
                                         al_map_rgba(0, 0, 0, 0),
                                         0, 0, 1, 1,
                                         -100, -100, 1, 1, 0);
        }
    }
    int _p1_tiles_drawn = 0;
    for (int di = 0; di < num_entries; di++) {
        if (draw_order[di].bottom > 0) continue; /* skip elevated */

        /* Tile limit knob: stop after N ground tiles drawn. */
        if (s_tile_limit > 0 && _p1_tiles_drawn >= s_tile_limit) break;
        _p1_tiles_drawn++;

        /* Per-tile hold cycle: each tile is its own isolated batch.
         * The pass-wide hold is suppressed when this knob is on. */
        if (s_hold_per_tile) al_hold_bitmap_drawing(true);
        ...
        /* `al_hold_bitmap_drawing` only batches bitmap draws.
         * `al_draw_line` / `al_draw_filled_polygon` (primitives addon)
         * draw IMMEDIATELY regardless of hold. So under the pass-wide
         * hold:
         *   - tile base bitmap is queued in batch
         *   - wall polygon is drawn now at the screen position
         *   - border line is drawn now
         *   - more tiles ...
         *   - end of pass: hold(false) flushes all queued bitmaps
         *
         * The bitmap flush at end draws on TOP of the primitives,
         * hiding height borders / wall edges. Adjacent tiles' diamond
         * edges (where transitions + border lines render at the same
         * sub-pixel position with bilinear vs line-AA coverage) also
         * produce a doubled-outline effect during camera motion.
         *
         * Fix: for tiles that need primitive draws (elevated walls,
         * height borders, hover highlight, grid overlay), step OUT of
         * the held batch: flush the queued bitmaps, draw the tile
         * fully in immediate mode (bitmaps + primitives interleaved
         * correctly), then re-enter the held batch for the next
         * batchable tile. The bulk of tiles (flat ground, no overlay,
         * no hover, no grid) stays inside the held batch unchanged. */
        /* Smooth-mode slope detection, computed HERE, before the hold-batch
         * decision below, because a sloped tile's top is drawn with
         * al_draw_prim (a primitives-addon call) which renders INCORRECTLY
         * inside al_hold_bitmap_drawing(true): the queued-batch state scales
         * + misplaces the warped quad (the "flat<->slope transition tiles black
         * out / shrink toward screen centre" bug). Such tiles must step out
         * of the held batch exactly like wall/hover/grid tiles already do. A
         * FLAT tile takes the batch-safe al_draw_tinted_scaled_bitmap path and
         * stays held. h_n/h_e/h_s/h_w are reused by the vx/vy block below so
         * iso_map_smooth_corner_heights is called only once per tile. */
        float h_n = (float)h, h_e = (float)h, h_s = (float)h, h_w = (float)h;
        int tile_is_flat = 1;
        if (map->smooth_height) {
            iso_map_smooth_corner_heights(map, mx, my, &h_n, &h_e, &h_s, &h_w);
            tile_is_flat = (h_n == h_e && h_e == h_s && h_s == h_w);
        }
        int tile_needs_primitives = (seg_top > 0) ||
                                    (map->smooth_height && !tile_is_flat) ||
                                    (!s_no_hover_grid && mx == map->hover_mx && my == map->hover_my) ||
                                    (map->show_grid && !s_no_hover_grid);
        int unbatched_tile = effective_hold_pass && tile_needs_primitives;
        if (unbatched_tile) {
        ...
        if (map->smooth_height) {
            /* h_n/h_e/h_s/h_w + tile_is_flat already computed above. */
        ...
        /* Top surface: ALWAYS set the blender at the start of every
         * tile, dropping any cross-tile cache. Under
         * `al_hold_bitmap_drawing(true)`, `al_set_blender` flushes the
         * current held batch and starts a new one. If the call were
         * skipped when the previous tile ended on ONE, the held
         * batch's flush boundaries would depend on which tiles had
         * overlays/walls/hover/grid, and that set shifts when the
         * camera moves. Inconsistent flush boundaries cause
         * inconsistent GPU-rasterised pixel coverage on shared
         * diamond edges, producing flickering darker borders during
         * camera motion. Forcing the call here makes the flush
         * boundary deterministic at every tile, trading O(1000) extra
         * al_set_blender calls per frame for stable rasterisation.
         * The within-tile transitions loop keeps its conditional `if
         * (!blender_is_one)` skip because within a tile the state is
         * fresh and skipping is safe. */
        if (s_force_alpha) {
        ...
        if (map->smooth_height && !tile_is_flat) {
            _draw_tile_warped(tile_bitmaps[top_terrain], vx, vy, tile_w, tile_h, seg_tint);
        } else {
            /* Flat tile (or CUT mode): the al_draw_prim warp samples atlas
             * sub-bitmaps differently from al_draw_tinted_scaled_bitmap, which
             * renders flat ground tiles wrong once tiles are packed into the
             * terrain atlas. For a flat tile the warp geometry is identical to
             * this scaled-bitmap draw anyway (verified: corner span == tdw*tdh),
             * so route flat tiles through the proven atlas-correct path and
             * reserve the warp for genuinely sloped tiles. */
            al_draw_tinted_scaled_bitmap(tile_bitmaps[top_terrain], seg_tint,
                                         0, 0, (float)tile_w, (float)tile_h,
                                         base_dx, base_dy, tile_draw_w, tile_draw_h, 0);
        }

        /* Cliff walls (internally sets ONE blender). */
        if (seg_top > 0) {
            if (map->smooth_height) {
                ...
                if ((h - h_east > map->smooth_slope_max) ||
                    ...
                    /* blender_is_one stays 1: function leaves state on ONE */
                }
            } else {
                _draw_segment_sides(map, vx[1], vy[1], vx[2], vy[2], vx[3], vy[3],
                                    mx, my, 0, seg_top, lift, seg_brightness, wall_terrain);
                /* blender_is_one stays 1 */
            }
        }

        /* Terrain transitions (same tint as base tile; ONE blender) */
        if (transition_tiles && !s_no_transitions) {
            ...
            for (int t = base + 1; t < nt; t++) {
                if (edge_bits_arr[t] != 0 && transition_tiles[t] &&
                    ...
                    if (!blender_is_one && !s_force_alpha) {
                        al_set_blender(ALLEGRO_ADD, ALLEGRO_ONE, ALLEGRO_INVERSE_ALPHA);
                        blender_is_one = 1;
                    }
                    if (map->smooth_height && !tile_is_flat) {
        ...
        /* Overlay tile (drawn after terrain transitions, before
         * walls/borders). Overlay bitmaps are premultiplied (loaded
         * through the same path as base tiles + transitions), so
         * ALPHA blending would produce quadratic darkening at
         * fractional-alpha edges. Use ONE/INVERSE_ALPHA which matches
         * the premul source format. Also avoids cross-tile
         * contamination under al_hold_bitmap_drawing(true), where
         * al_set_blender does not flush the held batch mid-pass: an
         * ALPHA blender set here would otherwise apply to draws
         * emitted earlier in the batch by other tiles when the batch
         * eventually flushes. */
        if (overlay_bitmaps && map->overlay && num_overlay_tiles > 0 && !s_no_overlay) {
        ...
        /* Height borders (ONE blender; lines use RGB=0 so premul == straight) */
        if (seg_top > 0 && !s_no_height_borders) {
            if (!blender_is_one) {
                al_set_blender(ALLEGRO_ADD, ALLEGRO_ONE, ALLEGRO_INVERSE_ALPHA);
                blender_is_one = 1;
            }
        ...
        /* Hover highlight (ONE blender; colors premultiplied below) */
        if (!s_no_hover_grid && mx == map->hover_mx && my == map->hover_my) {
            if (!blender_is_one) {
                al_set_blender(ALLEGRO_ADD, ALLEGRO_ONE, ALLEGRO_INVERSE_ALPHA);
                blender_is_one = 1;
            }
            /* Premul: each RGB channel scaled by alpha/255. */
            ALLEGRO_COLOR hc = player_mode
                                   ? al_map_rgba(100 * 120 / 255, 255 * 120 / 255, 100 * 120 / 255, 120)
                                   : al_map_rgba(255 * 120 / 255, 255 * 120 / 255, 100 * 120 / 255, 120);
            float verts[8] = {vx[0], vy[0], vx[1], vy[1],
                              vx[2], vy[2], vx[3], vy[3]};
            al_draw_filled_polygon(verts, 4, hc);
            ALLEGRO_COLOR oc = player_mode
                                   ? al_map_rgba(100 * 200 / 255, 255 * 200 / 255, 100 * 200 / 255, 200)
                                   : al_map_rgba(255 * 200 / 255, 255 * 200 / 255, 100 * 200 / 255, 200);
        ...
        /* Grid overlay (ONE blender; colors premultiplied below).
         * No blender_is_one = 1 update needed: this is the last block in
         * the iteration that touches the blender, and the next iteration
         * unconditionally re-sets the blender at the top of the loop. */
        if (map->show_grid && !s_no_hover_grid) {
            if (!blender_is_one) {
                al_set_blender(ALLEGRO_ADD, ALLEGRO_ONE, ALLEGRO_INVERSE_ALPHA);
            }
            /* Premul: white 60/255 alpha is (60,60,60,60). */
            ALLEGRO_COLOR gc = al_map_rgba(60, 60, 60, 60);
            ...
            if (h > 0) {
                unsigned char a2 = (unsigned char)(60 + h * 35);
                /* Premul: (R*a/255, G*a/255, B*a/255, a) for (255,200,100,a). */
                ALLEGRO_COLOR hc2 = al_map_rgba(a2,
        ...
        /* Per-tile hold cycle: flush this tile's batch before moving
         * on. Each tile becomes its own VBO. */
        if (s_hold_per_tile) al_hold_bitmap_drawing(false);

        /* Re-enter the held batch for the next batchable tile.
         * Resets the blender to ONE for the next tile's base bitmap,
         * since immediate-mode primitive draws inside the unbatched
         * section may have left other state.
         * No blender_is_one = 1 update: the next iteration unconditionally
         * re-sets the blender at the top of the loop. */
        if (unbatched_tile) {
            al_hold_bitmap_drawing(true);
            al_set_blender(ALLEGRO_ADD, ALLEGRO_ONE, ALLEGRO_INVERSE_ALPHA);
        }
    ...
    if (effective_hold_pass) al_hold_bitmap_drawing(false);
    n_iso_pass1_us += (al_get_time() - _iso_t_pass1) * 1.0e6;
    double _iso_t_pass2 = al_get_time();
    ...
    n_iso_pass2_us += (al_get_time() - _iso_t_pass2) * 1.0e6;
    double _iso_t_pass3 = al_get_time();
    /* Step 7: Occlusion detection and clipped overlay pass.
     * For each occluded entity, restrict the ghost draw to the region
     * below the occluding tile's north vertex using a clipping rectangle. */
    for (int i = 0; i < nobj; i++) {
        int oi = obj_order[i];
        ...
        if (!_iso_object_check_occlusion(map,
                                         ...
                                         objects[oi].sprite_h,
                                         objects[oi].sprite_half_w,
                                         cam_px, cam_py, zoom,
                                         &clip_y))
            continue;

        objects[oi].is_occluded = 1;
        objects[oi].occlude_clip_y = clip_y;

        /* Clip ghost overlay to below the occluder's top edge. The
         * ghost is drawn at alpha < 1 to make the occluded sprite
         * look semi-transparent. Use straight-alpha blending here:
         * with the premultiplied (ADD/ONE/INV_ALPHA) blender the
         * 0.4-alpha overlay is ADDED on top of the already-drawn
         * full-bright sprite, brightening it instead of revealing
         * what's behind. ALPHA/INV_ALPHA does the standard
         * src*alpha + dst*(1-alpha) transparent compositing the
         * caller meant. */
        int prev_cx, prev_cy, prev_cw, prev_ch;
        al_get_clipping_rectangle(&prev_cx, &prev_cy, &prev_cw, &prev_ch);
        int clip_top = (int)floorf(clip_y);
        if (clip_top < prev_cy) clip_top = prev_cy;
        al_set_clipping_rectangle(prev_cx, clip_top,
                                  prev_cw, prev_cy + prev_ch - clip_top);
        al_set_blender(ALLEGRO_ADD, ALLEGRO_ALPHA, ALLEGRO_INVERSE_ALPHA);
        objects[oi].draw(obj_sx_arr[oi], obj_sy_arr[oi],
    ...
    n_iso_pass3_us += (al_get_time() - _iso_t_pass3) * 1.0e6;

    /* The draw_order / obj_* buffers are file-static and reused
     * across calls; no per-call free. They survive for the process
     * lifetime which is fine: max draw entries is bounded by
     * map->width * map->height * ISO_MAX_SEGMENTS_PER_TILE (~2 KB
     * for typical 32x32x2 chunks), and obj_buf_cap grows once to the
     * scene's peak entity count. */
} /* iso_map_draw() */
