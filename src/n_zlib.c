 ...
 *@param nLenSrc The uncompressed source data length in bytes
...
size_t CompressData(unsigned char* abSrc, size_t nLenSrc, unsigned char* abDst, size_t nLenDst) {
    ...
    switch (nErr) {
        case Z_OK:
            ...
            break;
        default:
            n_log(LOG_ERR, "%s on string %p size %zu", zError(nErr), abSrc, nLenSrc);
            return 0;
    }
...
N_STR* zip_nstr(N_STR* src) {
    ...
    if (compressed_size == 0) {
        free_nstr(&zipped);
        n_log(LOG_ERR, "unable to zip string %p  %zu/%zu bytes", src->data, src->written, src->length);
        return NULL;
    }
    zipped->written = 4 + compressed_size;
    n_log(LOG_DEBUG, "zip :%zu original: %zu", zipped->written, src->length);
...
N_STR* unzip_nstr(N_STR* src) {
    ...
    if (src->written < 4) {
        n_log(LOG_ERR, "compressed data too short");
        return NULL;
    }
    ...
    if (original_size > 256 * 1024 * 1024) {
        n_log(LOG_ERR, "decompressed size too large: %u", original_size);
        return NULL;
    }
    /* storage for original string size + zipped string  + padding */
    ...
    /* copying size */
    unzipped->written = UncompressData(((unsigned char*)src->data) + 4, src->written - 4, (unsigned char*)unzipped->data, original_size);
    if (unzipped->written == 0) {
        n_log(LOG_ERR, "unable to unzip string %p  %zu/%zu bytes", unzipped->data, unzipped->written, unzipped->length);
        free_nstr(&unzipped);
        return NULL;
    }
    n_log(LOG_DEBUG, "Size: zip: %zu => unzip :%zu original:%u", src->written, unzipped->written, original_size);
