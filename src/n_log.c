...
int set_log_file(char* file) {
    ...
    __n_assert(fd != -1, n_log(LOG_ERR, "couldn't create %s with 0600 permission", _str(file)); return FALSE);

    log_file = fdopen(fd, "a");
    __n_assert(log_file, close(fd); n_log(LOG_ERR, "fdopen returned an invalid file descriptor pointer for %s:%d", _str(file), fd); return FALSE);
