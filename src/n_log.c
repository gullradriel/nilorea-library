...
typedef struct LOG_LEVELS {
    /*! string of log type */
    char* c_name;
    int c_val;     // cppcheck-suppress unusedStructMember ; used by downstream log consumers
    char* w_name;  // cppcheck-suppress unusedStructMember ; used by downstream log consumers
 ...
 *@param file File containing the emitted log
 *@param func Function emitting the log
    ...
    if (level <= log_level) {
        ...
        switch (LOG_TYPE) {
            case LOG_SYSJRNL:
                ...
                va_end(args);
                // Fallback if vasprintf is NULL */
                syslogsafebuffer = syslogbuffer ? syslogbuffer : "<log: vasprintf failed>";
#ifdef __windows__
                /* Sanitize syslog content to prevent command injection via system().
                 * Only mutate when syslogbuffer is non-NULL: syslogsafebuffer then
                 * points to heap memory.  When syslogbuffer is NULL it points to a
                 * read-only string literal and must not be written to. */
                if (syslogbuffer) {
                    for (char* p = syslogsafebuffer; *p; p++) {
                        if (*p == '"' || *p == '&' || *p == '|' || *p == '<' || *p == '>' || *p == '^') *p = '_';
                    }
                }
                int snprintf_ret = snprintf(NULL, 0, "start /B EventCreate /t %s /id 666 /l APPLICATION /so %s /d \"%s\" > NUL 2>&1", prioritynames[level].w_name, name, syslogsafebuffer);
                if (snprintf_ret > 0) {
                    needed = (unsigned long long)snprintf_ret;
                    Malloc(eventbuffer, char, needed + 4);
                    if (eventbuffer) {
                        snprintf(eventbuffer, needed + 4, "start /B EventCreate /t %s /id 666 /l APPLICATION /so %s /d \"%s\" > NUL 2>&1", prioritynames[level].w_name, name, syslogsafebuffer);
                        system(eventbuffer);
                    }
                }
#else
                syslog(level, "%s->%s:%d %s", _str(file), _str(func), line, syslogsafebuffer);
#endif
